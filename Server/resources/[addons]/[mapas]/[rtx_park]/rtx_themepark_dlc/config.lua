...
Config.Framework = "standalone"  -- Seoul/vRP uses the standalone path; integration is handled in server/other.lua
...
Config.OxInventory = true -- Seoul uses ox_inventory

Config.InterfaceColor = "#ff66ff" -- change interface color, color must be in hex

Config.Language = "Portuguese" -- Seoul BR
...
Config.Target = true -- Seoul uses ox_target

Config.Targettype = "oxtarget" -- Seoul target
...
Config.ServerSideObjectsOnly = true -- enable this feature only if you are using entity lockdown (strict mode) on the server, all objects will be created via serverside.
...
function Notify(text)
	TriggerEvent("Notify", "azul", text, 5000)
end

function AddBumperKey(vehicle, model, plate)
	-- Seoul: bumper cars are temporary and must not create a permanent inventory key.
	-- The core/garage accepts a vehicle when Entity(vehicle).state.Lockpick matches the EXACT GTA plate.
	if not IsDuplicityVersion() and vehicle and DoesEntityExist(vehicle) then
		local realPlate = GetVehicleNumberPlateText(vehicle)
		if realPlate and realPlate ~= "" then
			Entity(vehicle).state:set("Lockpick",realPlate,true)
		end
...
function RemoveBumperKey(vehicle, model, plate)
	-- No persistent key is created, so there is nothing to remove.
end

function IncreaseHunger(itemtype)
	if not IsDuplicityVersion() and (itemtype == "hotdog" or itemtype == "burger" or itemtype == "juice") then
		TriggerServerEvent("rtx_themepark:Seoul:Consume",itemtype)
	end
end
