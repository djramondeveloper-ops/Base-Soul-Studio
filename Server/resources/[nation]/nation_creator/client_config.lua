...
creatorCoords = vec3(-772.83,342.53,211.4)
creatorHeading = 87.88
-- 229.61

spawnCoords = vec3(-1038.01,-2737.37,20.17)
spawnHeading = 274.97
...
resetCloths = function()
    ...
    local data = clothesData[model]
    if not data then return end

    for _, comp in ipairs(data.components) do
        local componentId, drawable, texture = table.unpack(comp)
    ...
    for _, prop in ipairs(data.props) do
        local propId, drawable, texture = table.unpack(prop)
...
function setPlayersVisible(bool)
    ...
    SetEntityInvincible(ped, not bool) -- COMENTE ESTA LINHA SE HOUVER PROBLEMAS COM ANTI-CHEAT
...
function setPlayerInvisibleToNetwork(state)
    local ped = PlayerPedId()
    NetworkSetEntityInvisibleToNetwork(ped, state) 
...
function initCreator()
    loadingPlayer(true)
    TriggerEvent('hud:Active2', false)
    ...
    setPlayerInvisibleToNetwork(true)
...
function finishCreator()
    ...
    setPlayerInvisibleToNetwork(false)
...
function setGender(gender)
    ...
    -- Salvar dados importantes do ped atual
    ...
    -- Esperar um pouco para o modelo se aplicar
    Wait(100)

    ped = PlayerPedId() -- atualizar ped depois da troca
    SetPedMaxHealth(ped, maxHealth)
    SetEntityHealth(ped, currentHealth)
    SetPedArmour(ped, currentArmour)

    -- Restaurar roupas e dados de aparência
    resetCloths()

    -- Evita problemas visuais definindo head blend corretamente
    if gender == "male" then
    ...
    -- Reaplica visibilidade (caso o modelo bugue)
    SetEntityVisible(ped, true, false)
