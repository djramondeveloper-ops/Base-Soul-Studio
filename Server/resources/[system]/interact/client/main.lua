...
local hidden = require 'client.modules.hidden'
...
local function hexToRgba(hex, alpha)
    ...
    return {
        tonumber(hex:sub(1, 2), 16) or r,
        tonumber(hex:sub(3, 4), 16) or g,
        tonumber(hex:sub(5, 6), 16) or b,
        alpha or a or 200
    }
end

local function themeColorFromState(theme, basics)
    local payload = type(theme) == 'table' and theme or {}
    theme = type(payload.Theme) == 'table' and payload.Theme or type(payload.SeoulTheme) == 'table' and payload.SeoulTheme or payload
    basics = type(basics) == 'table' and basics or type(payload.Basics) == 'table' and payload.Basics or GlobalState.Basics or {}

    return theme.Primary or theme.PrimaryColor or theme.CityColorHex or theme.Main or theme.main or theme.color or basics.CityColorHex or basics.CityColor
end

local function applySeoulInteractTheme(theme, basics)
    local color = hexToRgba(themeColorFromState(theme, basics), 200)
...
RegisterNetEvent('Seoul:ThemeUpdated', function(theme)
    applySeoulInteractTheme(theme, GlobalState.Basics)
end)

RegisterNetEvent('Seoul:AdminControl:ApplyClientConfig', function(configOrPayload, theme, basics)
    applySeoulInteractTheme(theme or configOrPayload, basics)
end)

AddStateBagChangeHandler('SeoulTheme', 'global', function(_, _, value)
    Wait(150)
    applySeoulInteractTheme(value, GlobalState.Basics)
end)

AddStateBagChangeHandler('Basics', 'global', function(_, _, value)
    Wait(150)
    applySeoulInteractTheme(GlobalState.SeoulTheme, value)
end)

CreateThread(function()
    Wait(1500)
    applySeoulInteractTheme(GlobalState.SeoulTheme, GlobalState.Basics)
end)
...
local function shouldHideInteract()
    if IsNuiFocused() or LocalPlayer.state.hideInteract or (lib and lib.progressActive()) or hidePerKeybind then
        return true
    end
    -- Suporte para ox_inventory (invOpen) e inventário customizado (inInventory)
    if LocalPlayer.state.invOpen or LocalPlayer.state.inInventory then
        return true
    end
    return false
end
            ...
            if data and data.coords and not data.hideCompletely then
                ...
                if not foundValid and data.validOpts and data.validCount > 0 then
                    ...
                    if distance < config.maxInteractDistance and item.currentScreenDistance < math.huge then
                        local t = distance / config.maxInteractDistance
                        local newSize = config.minInteractionSize + (config.maxInteractionSize - config.minInteractionSize) * (t ^ 0.7)

                        DrawSprite(hidden.instance.dictName, hidden.instance.txtName, 0.0, 0.0, newSize * aspectRatio, newSize * aspectRatio, 0.0, 255, 255, 255, 255)
                    end
                end
