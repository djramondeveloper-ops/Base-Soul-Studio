...
local function addOptions(target, options, resource, bonesTarget, offsetsTarget)
    ...
    resource = resource or 'interact'

    for i = #options, 1, -1 do
        local option = options[i]
        option.resource = option.resource or resource

        if resource == 'interact' then
...
function interact.removeCoords(id, remove)
    ...
    if (idType == 'vector3' or idType == 'vector4') then
        id = utils.makeIdFromCoords(id)
    end

    if not store.coords[id] then return end
