...
export function setDefaultColor(color) {
  ...
  if (parts && parts.length >= 3) {
    defaultSoftColor = `rgb(${parts[0]}, ${parts[1]}, ${parts[2]}, 0.35)`;
  }
  return defaultColor;
}
...
export function updateHighlight() {
  ...
  if (options.length > 0) {
    ...
    if (options[currentIndex].color) {
      ...
      document.body.style.setProperty("--theme-color-soft", softColor);
    } else {
      if (defaultColor) document.body.style.setProperty("--theme-color", defaultColor);
      if (defaultSoftColor) document.body.style.setProperty("--theme-color-soft", defaultSoftColor);
    }
  }
}
