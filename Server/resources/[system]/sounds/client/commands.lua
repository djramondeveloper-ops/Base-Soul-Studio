...
RegisterCommand("streamermode", function(source, args, rawCommand)
    ...
    TriggerEvent("sounds:streamerMode", disableMusic)
    if disableMusic then
        TriggerEvent('chat:addMessage', { args = { "^1[sounds]", config.Messages["streamer_on"] } })
    else
        TriggerEvent('chat:addMessage', { args = { "^1[sounds]", config.Messages["streamer_off"] } })
    end
end, false)

AddEventHandler("sounds:streamerMode", function(status)
    if status then
