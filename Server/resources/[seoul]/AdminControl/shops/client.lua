...
local shopInteractTargets = {}
local rebuildInteractShops

AddStateBagChangeHandler("AllShops","",function (_,_,value)
    Shops = value or {}
    if rebuildInteractShops then rebuildInteractShops() end
end)
...
local function clearInteractShops()
    if not isInteractReady() then
        shopInteractTargets = {}
        return
    end

    for _,id in pairs(shopInteractTargets) do
        pcall(function() exports.interact:removeCoords(id) end)
    end
    shopInteractTargets = {}
end

rebuildInteractShops = function()
    clearInteractShops()
    if not isInteractReady() then return end

    for k,shop in pairs(Shops or {}) do
        if shop.locations then
            for i,shopCoords in pairs(shop.locations) do
                local name = ("AdminControl:shop:%s:%s"):format(tostring(k),tostring(i))
                shopInteractTargets[name] = exports.interact:addCoords(vector3(shopCoords.x,shopCoords.y,shopCoords.z),{
                    name = name,
                    label = "Abrir "..(shop.name or "Loja"),
...
CreateThread(function ()
    if GlobalState['Inventory'] == "ox_inventory" then
        ...
        AllItems = GlobalState["SeoulConfig"].items
    end
end)
