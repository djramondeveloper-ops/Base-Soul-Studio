...
local elevatorInteractTargets = {}
local rebuildInteractElevators

AddStateBagChangeHandler("Elevators","",function (_,_,value)
    Elevators = value
    DrawCoords = {}
    for _, k in pairs(Elevators or {}) do
        for _, v in pairs(k.Floors) do
            table.insert(DrawCoords, v['Coords'])
        end
    end
    if rebuildInteractElevators then rebuildInteractElevators() end
end)
...
local function clearInteractElevators()
    if not isInteractReady() then
        elevatorInteractTargets = {}
        return
    end

    for _,id in pairs(elevatorInteractTargets) do
        pcall(function() exports.interact:removeCoords(id) end)
    end
    elevatorInteractTargets = {}
end

rebuildInteractElevators = function()
    clearInteractElevators()
    if not isInteractReady() then return end

    for i = 1,#DrawCoords do
        local coords = DrawCoords[i]
        if coords then
            local name = "AdminControl:elevator:"..tostring(i)
            elevatorInteractTargets[name] = exports.interact:addCoords(vector3(coords.x,coords.y,coords.z),{
                name = name,
                label = "Abrir elevador",
