local AllPeds = GlobalState['AllPeds'] or {}
local pedIndex = 1
...
local function createLocalPed(model,coords,heading)
    ...
    if hash and LoadModel(hash) then
        local nped = CreatePed(4,hash,coords.x, coords.y, coords.z + 0.1,heading,false,false)
...
local function managePed(args)
    local PedIndex = tonumber(args.pedIndex) or args.pedIndex
    if not AllPeds or not AllPeds[PedIndex] then
        TriggerEvent("Notify","negado","Ped nao encontrado.",5000)
        return
    end
...
local function listPeds()
    AllPeds = GlobalState['AllPeds'] or AllPeds or {}
...
function GetPedCoords()
    if not Config.PedHashs or #Config.PedHashs <= 0 then
        TriggerEvent("Notify","negado","Nenhum modelo de ped configurado.",5000)
        return nil
    end
