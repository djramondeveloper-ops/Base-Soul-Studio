...
function Server.registerStash(Stash)
    local source = source
    if not AdminControlCanUse(source, Config.Commands["stashes"].perm) then return end
    if Stash.label then
        local Stashes = GlobalState['AllStashes']
        local id = #Stashes + 1
...
function Server.deleteStash(index)
    ...
    if Stashes[index] then
        ...
        exports.oxmysql:query_async("DELETE FROM ox_inventory WHERE name = ?",{ Stashes[index].id })
...
local function getStashName(id)
    local Stashes = GlobalState["AllStashes"] or {}
...
local loaded = false
local function registerStashHooks()
    if loaded or GetResourceState("ox_inventory") ~= "started" then return end
    Wait(500)
    if loaded or GetResourceState("ox_inventory") ~= "started" then return end
    loaded = true
    exports.ox_inventory:registerHook('swapItems', function(payload)
            ...
            local user_id = AdminControlPassport(tonumber(payload.source))
            local toInvSplit = splitString(tostring(payload.toInventory or ""),":")
            local fromInvSplit = splitString(tostring(payload.fromInventory or ""),":")
            ...
            elseif payload.fromInventory and tostring(payload.fromInventory):find("drop") then
            ...
            elseif payload.toType == "player" and payload.fromType == "player" then
                ...
                title = ("ID (%s) PEGOU DO ID (%s)"):format(tostring(payload["toInventory"]), tostring(payload["fromInventory"]))
