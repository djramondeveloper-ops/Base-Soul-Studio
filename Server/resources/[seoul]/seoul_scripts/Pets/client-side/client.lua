if SeoulScriptsClient and not SeoulScriptsClient.Enabled('Pets') then return end
...
AddEventHandler("dynamic:animalSpawn",function(model)
	...
	if SeoulScripts and SeoulScripts.Pets and SeoulScripts.Pets.AllowedModels and not SeoulScripts.Pets.AllowedModels[model] then
		TriggerEvent("Notify","negado","Pet não permitido pela configuração da Seoul.",5000)
		return
	end
