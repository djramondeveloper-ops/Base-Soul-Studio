...
local function copsAmount()
    if vRP.AmountService then
        return parseInt(vRP.AmountService(Config.PolicePermission or "Policia"))
    end
    if vRP.NumPermission then
        local _,amount = vRP.NumPermission(Config.PolicePermission or "Policia")
...
local function policeSources()
    local list = {}
    if vRP.NumPermission then
        local service = vRP.NumPermission(Config.PolicePermission or "Policia")
...
function API.finishRegister(x,y,z)
    ...
    giveItem(passport,Config.PaymentItem or "dirtydollar",parseInt(Config.cashMachine.machine.payment or 300))
...
function API.stockadeWithdraw(vehPlate,vehNet)
    ...
    if stockadePlates[vehPlate] > 0 then
        ...
        TriggerClientEvent("cancelando",source,false)
        giveItem(passport,Config.stockade.payment.item,parseInt(Config.stockade.payment.qntd or math.random(15000,30000)))
        if stockadePlates[vehPlate] > 0 then
