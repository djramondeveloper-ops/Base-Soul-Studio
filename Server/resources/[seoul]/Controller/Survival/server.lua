if Config.Modules and Config.Modules.Survival == false then return end
...
-- GOD
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("god",function(source,args,rawCommand)
	if source == 0 then
...
-- GOOD
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("good",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- REVIVE
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("re",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
...
RegisterNetEvent("Controller:paramedic:Revive")
...
-- COMMANDOS
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("finalizar",function(source,args,rawCommand)
	local nplayer = vRPclient.nearestPlayer(source,2)
	if SvTunnel.Death(nplayer) then
...
SeoulRegisterCommand("bugado",function(source,args,rawCommand)
...
-- SOCORRO
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("socorro",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
...
function SvServer.callMedics()
    ...
    if vRP.AmountService("Hospital") > 0 then
		local players = vRP.getUsersByPermission("paramedico.permissao") or {}
