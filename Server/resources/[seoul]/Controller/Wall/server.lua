if Config.Modules and Config.Modules.Wall == false then return end
...
function WallServer.getInfos()
	local caller = source
	local passport = vRP.getUserId(caller)
	if not passport or not HasPermission(caller,"wall") then return {} end
	local players = {}
...
function WallServer.reportLog(toggle)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id and HasPermission(source,"wall") then
		if toggle == "ON" then
