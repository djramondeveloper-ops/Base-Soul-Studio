...
if LocalPlayer.state.Route == nil then LocalPlayer["state"]:set("Route",0,true) end
-----------------------------------------------------------------------------------------------------------------------------------------
-- RECEIVESALARY
-----------------------------------------------------------------------------------------------------------------------------------------
if Config.Features and Config.Features.LegacySalary then
...
-- CUFF
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("keybindcuff",function(source,args)
...
-- CARRY
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("keybindcarry",function(source,args)
...
-- VTUNING
-----------------------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("vtuning",function(source,args)
	local ped = PlayerPedId()
...
SeoulRegisterCommand("sequestro2",function(source,args)
	local complet = false
	local ped = PlayerPedId()
...
-- FPS ON & OFF 
------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("fps",function(source,args)
    if args[1] == "on" then
...
-- CRUISER
------------------------------------------------------------------------------------------------------------------------
SeoulRegisterCommand("cruiser",function(source,args)
	local ped = PlayerPedId()
	local veh = GetVehiclePedIsUsing(ped)
					...
					if distance <= 1.5 then
						...
						if IsControlJustPressed(1,38) then
							...
							if v.joinBucket then
								TriggerServerEvent("Controller:TeleportBucket","Enter",v.joinBucket)
							elseif v.exitBucket then
								TriggerServerEvent("Controller:TeleportBucket","Exit",v.exitBucket)
							end
