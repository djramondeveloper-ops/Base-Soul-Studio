...
local function openNearbyInventory()
	-- Seoul: desativado para não tentar abrir inventário de player próximo e gerar
	-- 'You can not open this inventory'. Use ox_target para revista/roubo.
	return false
end
...
local function registerCommands()
	...
	local secondaryKey = client.keys[2]
	if secondaryKey and secondaryKey ~= '' and secondaryKey:upper() ~= 'NONE' and secondaryKey:upper() ~= 'DISABLED' then
		lib.addKeybind({
			name = 'inv2',
			description = locale('open_secondary_inventory'),
			defaultKey = secondaryKey,
	...
	lib.addKeybind({
		...
		defaultKey = client.keys[3] or 'TAB',
		onPressed = function()
