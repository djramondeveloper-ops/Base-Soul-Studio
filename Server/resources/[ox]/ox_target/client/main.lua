...
local function normalizeSeoulHex(value, fallback)
    value = tostring(value or fallback or '#5bb894'):gsub('%s+', '')
    ...
    return fallback or '#5bb894'
end

local function getSeoulTargetTheme(payload)
...
local function sendSeoulTargetTheme(payload)
    SendNuiMessage(json.encode(getSeoulTargetTheme(payload)))
end

RegisterNetEvent('Seoul:AdminControl:ApplyClientConfig', function(payload)
    Wait(150)
    sendSeoulTargetTheme(payload)
end)

AddStateBagChangeHandler('SeoulTheme', 'global', function(_, _, value)
    Wait(150)
    sendSeoulTargetTheme(value)
end)

CreateThread(function()
    Wait(1500)
    sendSeoulTargetTheme()
end)

local function isValidTargetEntity(entity)
    return type(entity) == 'number' and entity > 0 and DoesEntityExist(entity)
end

local function getSafeEntityType(entity)
    if not isValidTargetEntity(entity) then return 0 end

    local success, result = pcall(GetEntityType, entity)
    return success and result or 0
end

local function getSafeEntityModel(entity)
    if not isValidTargetEntity(entity) then return nil end

    local success, result = pcall(GetEntityModel, entity)
    return success and result or nil
end

local function hasSafeLineOfSight(entity)
    if not isValidTargetEntity(entity) or not isValidTargetEntity(cache.ped) then return false end

    local success, result = pcall(HasEntityClearLosToEntity, entity, cache.ped, 7)
    return success and result or false
end

local function setSafeOutline(entity, enabled)
    if debug and isValidTargetEntity(entity) then
        pcall(SetEntityDrawOutline, entity, enabled)
    end
end

local function stripOptionForNui(option)
...
local function stripOptionsForNui(source)
    ...
    for key, value in pairs(source) do
        if type(value) == 'table' then
            ...
            for optionKey, optionValue in pairs(value) do
                optionList[optionKey] = stripOptionForNui(optionValue)
            end
...
local function startTargeting()
    ...
    while state.isActive() do
        ...
        if entityHit ~= 0 and entityHit ~= lastEntity then
            entityType = getSafeEntityType(entityHit)
        ...
        if isValidTargetEntity(entityHit) and entityChanged then
            currentMenu = nil

            if flag ~= 511 and not hasSafeLineOfSight(entityHit) then
                entityHit = 0
                entityType = 0
                entityModel = nil
            end
            ...
            if lastEntity ~= entityHit and debug then
                setSafeOutline(lastEntity, false)

                if entityType ~= 1 then
                    setSafeOutline(entityHit, true)
                end
            end

            entityModel = getSafeEntityModel(entityHit)
        ...
        if newOptions then
            ...
            elseif menuChanged or hasTarget ~= 1 and hidden ~= totalOptions then
                ...
                SendNuiMessage(json.encode({
                    event = 'setTarget',
                    options = stripOptionsForNui(options),
                    zones = stripOptionsForNui(zones),
                }, { sort_keys = true }))
            end
