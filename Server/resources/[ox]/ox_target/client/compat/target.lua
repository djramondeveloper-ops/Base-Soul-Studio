...
local function convert(options)
    local distance = options.distance or options.Distance
...
RegisterNetEvent("seoul:target:enterTrunk")
AddEventHandler("seoul:target:enterTrunk",function(data)
    ...
    if IsThisModelABike(model)
        or GetEntityBoneIndexByName(entity,"boot") == -1
        or IsPedArmed(ped,7)
        or GetVehicleDoorLockStatus(entity) > 1 then
        return
    end
    ...
    TriggerEvent("player:enterTrunk",selected)
end)
