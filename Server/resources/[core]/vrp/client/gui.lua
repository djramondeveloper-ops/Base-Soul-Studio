...
-- WALKERS
-----------------------------------------------------------------------------------------------------------------------------------------
local Walkers = { "move_m@casual@a","move_m@casual@b","move_m@casual@c","move_m@casual@d","move_m@casual@e","move_m@casual@f","move_f@sexy@a","move_f@posh@","move_f@arrogant@a","move_f@femme@","move_f@heels@c","move_f@sassy","move_f@scared","move_f@hurry@a","move_f@tough_guy@","move_m@brave","move_m@confident","move_m@money","move_m@swagger","move_m@tough_guy@","move_m@shadyped@a","move_m@intimidation@cop@unarmed","move_m@business@a","move_m@business@b","move_m@business@c","move_m@gangster@generic","move_m@gangster@ng","move_m@gangster@var_e","move_m@gangster@var_f","move_m@gangster@var_i","move_m@drunk@a","move_m@drunk@slightlydrunk","move_m@drunk@verydrunk","move_m@buzzed","move_m@hobo@a","move_m@sad@a","move_p_m_zero","move_p_m_zero_janitor","move_p_m_zero_slow","move_p_m_two","move_m@quick","move_m@jog@","female_fast_runner","move_m@alien","move_characters@jimmy@slow@","move_characters@franklin@fire","move_characters@michael@fire","move_m@fire","anim@move_m@grooving@","anim_group_move_ballistic","anim_group_move_lemar_alley" }
-----------------------------------------------------------------------------------------------------------------------------------------
-- STATEBAGHANDLER
-----------------------------------------------------------------------------------------------------------------------------------------
AddStateBagChangeHandler("Walk", ("player:%s"):format(LocalPlayer.state.Source),
	function(bagName, key, value)
		local Ped = PlayerPedId()
		if not DoesEntityExist(Ped) then return end

		if value and LoadMovement(value) then
			SetPedMovementClipset(Ped, value, 0.5)
			CurrentWalk = value
		else
...
-- VRP:WALK
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("vRP:Walk")
AddEventHandler("vRP:Walk", function(Walk)
	if not Walk then
		LocalPlayer.state:set("Walk", nil, true)
		return
	end

	if type(Walk) ~= "string" then return end

	LocalPlayer.state:set("Walk", Walk, true)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADBLOCK
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("Engine",function()
	...
	if Timer < Button then
		local Cooldown = math.ceil((Button - Timer) / 1000)
		TriggerEvent("Notify","Atenção","Aguarde <b>"..CompleteTimers(Cooldown).."</b> para usar novamente.","amarelo",5000)
		return
	end

	if LocalPlayer["state"]["Active"] and not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(Ped) then
		...
		if Vehicle and GetPedInVehicleSeat(Vehicle,-1) == Ped then
			...
			if vRPS.HasVehicleKey(Plate) or Entity(Vehicle)["state"]["Lockpick"] == Plate then
				...
				if Running then
					...
					TriggerEvent("Notify","Atenção","O veículo foi <b>Desligado</b>.","amarelo",5000,false,false,false,true)
				else
					SetVehicleUndriveable(Vehicle,false)
					TriggerEvent("Notify","Sucesso","O veículo foi <b>Ligado</b>.","verde",5000,false,false,false,true)
				end
			else
				TriggerEvent("Notify","Aviso","Você não tem a chave deste veículo.","vermelho",5000)
			end
		end
	end
end)
...
RegisterCommand("NumBinds",function(source,Message)
	...
	if Message[1] and (Binded[Message[1]] or Message[1] == "0") and LocalPlayer["state"]["Active"] and GetGameTimer() >= Button and not IsPauseMenuActive() and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and GetEntityHealth(Ped) > 100 and not LocalPlayer["state"]["Cancel"] and not IsPedReloading(Ped) and not LocalPlayer["state"]["Handcuff"] and not IsPedInAnyVehicle(Ped) and not IsPedArmed(Ped,7) and not IsPedSwimming(Ped) then
		...
		if Message[1] == "0" then
			...
			TriggerEvent("animations:Emotes",Binded[Message[1]])
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
