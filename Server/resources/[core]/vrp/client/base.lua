...
-- LOADTEXTURE
-----------------------------------------------------------------------------------------------------------------------------------------
local function LoadTexture(Resource,FileName,TXD,Name,Size)
	Size = Size or 512

	local PngData = LoadResourceFile(Resource, "config/textures/"..FileName..".png")
	if not PngData then return nil end

	local Dict = "data:image/png;base64,"..Base64(PngData)
	local Texture = CreateRuntimeTexture(TXD,Name,Size,Size)
...
CreateThread(function()
	...
	local DirtTextures = {
		vehicle_genericmud_car = "mud_texture",
		vehicle_generic_tyrewall_dirt = "vehicle_generic_tyrewall_dirt"
	}

	local DirtTxd = CreateRuntimeTxd("customVehicleDirt")

	for OriginalName,ImageName in pairs(DirtTextures) do
		LoadTexture("vrp",ImageName,DirtTxd,OriginalName)
		AddReplaceTexture("vehshare",OriginalName,"customVehicleDirt",OriginalName)
	end
