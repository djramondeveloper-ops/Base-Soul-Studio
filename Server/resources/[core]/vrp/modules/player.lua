...
AddEventHandler("CharacterChosen",function(Passport,source,Creation)
	...
	TriggerEvent("Seoul:PlayerLoaded",Passport,source)
	...
	local WalkStyle = vRP.SingleQuery("playerdata/GetData",{ Passport = Passport, Name = "Walk" })
	if WalkStyle and WalkStyle.Information then
		local Clipset = json.decode(WalkStyle.Information)
		if type(Clipset) == "string" then
			TriggerClientEvent("vRP:Walk",source,Clipset)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
