...
vRP.Prepare("characters/UpdateLocate","UPDATE characters SET Locate = @Locate WHERE id = @Passport")
...
vRP.Prepare("vehicles/addVehicles","INSERT IGNORE INTO vehicles (Passport,Vehicle,Plate,Weight,Work,Tax,Save) VALUES (@Passport,@Vehicle,@Plate,@Weight,@Work,UNIX_TIMESTAMP() + (86400 * 7),@Save)")
...
vRP.Prepare("vehicles/rentalVehicles","INSERT IGNORE INTO vehicles (Passport,Vehicle,Plate,Weight,Work,Rental,Tax,Save) VALUES (@Passport,@Vehicle,@Plate,@Weight,@Work,UNIX_TIMESTAMP() + (86400 * @Days),UNIX_TIMESTAMP() + (86400 * @Days),@Save)")
...
-- LB-PHONE
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.Prepare("Hensa/PhoneCalls","DELETE FROM phone_phone_calls WHERE timestamp < NOW() - INTERVAL 3 DAY")
vRP.Prepare("Hensa/PhoneMessages","DELETE FROM phone_message_messages WHERE timestamp < NOW() - INTERVAL 3 DAY")
vRP.Prepare("Hensa/PhoneServices","DELETE FROM phone_services_messages WHERE timestamp < NOW() - INTERVAL 1 DAY")
vRP.Prepare("Hensa/PhoneNotifications","DELETE FROM phone_notifications WHERE timestamp < NOW() - INTERVAL 3 DAY")
vRP.Prepare("Hensa/PhoneStorys","DELETE FROM phone_instagram_stories_views WHERE timestamp < NOW() - INTERVAL 3 DAY")
vRP.Prepare("Hensa/PhoneInstagram","DELETE FROM phone_instagram_notifications WHERE timestamp < NOW() - INTERVAL 3 DAY")
-----------------------------------------------------------------------------------------------------------------------------------------
-- HEALTH
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.Prepare("health/Get","SELECT Illness FROM Illness WHERE Passport = @Passport")
vRP.Prepare("health/Set","REPLACE INTO illness(Passport,Illness) VALUES(@Passport,@Illness)")
vRP.Prepare("health/Remove","DELETE FROM illness WHERE Passport = @Passport")
-----------------------------------------------------------------------------------------------------------------------------------------
-- RACES
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- REDEEMCODES
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.Prepare("RedeemCodes/GetCode","SELECT * FROM redeem_codes WHERE Code = @Code AND Active = 1")
vRP.Prepare("RedeemCodes/GetAll","SELECT * FROM redeem_codes WHERE Active = 1 ORDER BY Created DESC")
vRP.Prepare("RedeemCodes/Create","INSERT INTO redeem_codes (Code,Rewards,Uses,MaxUses,Expires,Created,Active) VALUES (@Code,@Rewards,0,@MaxUses,@Expires,UNIX_TIMESTAMP(),1)")
vRP.Prepare("RedeemCodes/UpdateUses","UPDATE redeem_codes SET Uses = Uses + 1 WHERE Code = @Code")
vRP.Prepare("RedeemCodes/Deactivate","UPDATE redeem_codes SET Active = 0 WHERE Code = @Code")
vRP.Prepare("RedeemCodes/CheckExpired","SELECT * FROM redeem_codes WHERE Expires > 0 AND Expires <= UNIX_TIMESTAMP() AND Active = 1")
vRP.Prepare("RedeemCodes/DeactivateExpired","UPDATE redeem_codes SET Active = 0 WHERE Expires > 0 AND Expires <= UNIX_TIMESTAMP() AND Active = 1")
-----------------------------------------------------------------------------------------------------------------------------------------
-- REDEEMEDCODES
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.Prepare("RedeemedCodes/CheckRedeemed","SELECT * FROM redeemed_codes WHERE Code = @Code AND Passport = @Passport")
vRP.Prepare("RedeemedCodes/AddRedeemed","INSERT INTO redeemed_codes (Code,Passport,RedeemedAt) VALUES (@Code,@Passport,UNIX_TIMESTAMP())")
vRP.Prepare("RedeemedCodes/GetUserRedeemed","SELECT * FROM redeemed_codes WHERE Passport = @Passport ORDER BY RedeemedAt DESC")
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSERVERSTART
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
	...
	-- LB-PHONE
	if UsingLB then
		vRP.Query("Hensa/PhoneCalls")
		vRP.Query("Hensa/PhoneStorys")
		vRP.Query("Hensa/PhoneMessages")
		vRP.Query("Hensa/PhoneServices")
		vRP.Query("Hensa/PhoneInstagram")
		vRP.Query("Hensa/PhoneNotifications")

		if tostring(GetConvar("seoul:debug", "false")):lower() == "true" then print("^3[Seoul]^7 Códigos do LB Phone foram ativados.") end
	end
end)