...
function GetPlayerIdentifier()
    -- FIX 11: add a 30-second timeout so callers don't deadlock if the server
    -- never fires rcore_fuel:requestPlayerIdentifier (framework not ready, etc.)
    local waited = 0
    while not playerIdentifier do
        Wait(1000)
        waited = waited + 1
        if waited >= 30 then
            -- Return a local fallback so dependent code can still proceed
            playerIdentifier = "pending_" .. tostring(GetPlayerServerId(PlayerId()))
            break
        end
    end
