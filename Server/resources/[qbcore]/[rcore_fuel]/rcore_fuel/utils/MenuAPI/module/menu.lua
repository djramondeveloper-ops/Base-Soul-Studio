...
function CreateMenu(identifier)
    ...
    --------------
    self.Open = function()
        -- FIX 1: only one menu is ever visually shown at a time, but CachedMenu
        -- accumulates every menu ever opened -- if this Open() happens from inside
        -- another still-"open" menu's callback (a common pattern: menu A's item
        -- selection opens menu B), menu A was never marked closed. CloseAll() (ESC)
        -- would then also fire menu A's OnCloseEvent/OnExitEvent even though the
        -- player already moved past it, and IsAnyMenuOpen() could get stuck true.
        -- Silently clear other menus' open flag (no event fire -- this isn't a
        -- real cancel, just a step transition) before opening this one.
        for otherIdentifier, cached in pairs(CachedMenu) do
            if otherIdentifier ~= identifier and cached.self and cached.self.isOpen_ then
                cached.self.isOpen_ = false
            end
        end
