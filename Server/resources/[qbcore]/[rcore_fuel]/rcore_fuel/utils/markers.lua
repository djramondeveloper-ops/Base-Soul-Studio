...
end, "adding to the cache markers v2 - 9")
...
CreateThread(function()
    ...
    while true do
        local sleep = 500
        if isNearSomething then
            for i, self in pairs(nearMarkersV2) do
                if self.onlyVehicle == nil or IsPedAnyVehicle(getPed(), false) == self.onlyVehicle then
                    if not self.stopRendering and self.rendering and not self.destroyed and canRender then
                        sleep = 0
                        if self.isIn then
                        ...
                        if self.get3DText() ~= nil then
                            draw3DText(self.position - self.get3DTextOffset(), self.get3DText(), {
...
end, "render thread for markers v2 - 74")
...
function SetActiveRenderMarkers(result)
    canRender = result
end
...
function draw3DText(pos, text, options)
    ...
    local scaleOption = options.size or 1.4
...
function createMarker(res)
    ...
    self.FloatTextoffset = vector3(0, 0, 0)


    self.setTextColor = function(tbl)
        self.textColor = tbl
    end
    ------------------------------
    self.get3DTextOffset = function()
        return self.FloatTextoffset
    end
    ------------------------------
    self.set3DTextOffset = function(coords)
        self.FloatTextoffset = coords
    end
    ------------------------------
    self.get3DText = function()
        return self.FloatingText
    end
    ------------------------------
    self.set3DText = function(text)
