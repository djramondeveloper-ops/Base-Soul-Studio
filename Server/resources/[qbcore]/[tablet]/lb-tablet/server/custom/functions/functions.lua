...
-- Seoul integration: expose the existing billing implementation to other resources.
...
function HasPermission(source, job, permission, permissionType)
    ...
    local permissionGrade = playerJob.permission_grade or playerJob.grade

    if permissionGrade >= jobPermissions[permission][permissionType] then
        return true
    end
