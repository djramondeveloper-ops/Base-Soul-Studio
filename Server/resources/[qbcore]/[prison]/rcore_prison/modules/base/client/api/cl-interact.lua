...
Interact.LoadNPC = function(place, id)
    ...
    dbg.debug("Creating ped %s (%s) - vec3(%s, %s, %s)", entity, GetEntityArchetypeName(entity), coords.x, coords.y, coords.z)
...
function RegisterZones()
    if zonesRegistered then
        return dbg.debug('Zones already registered, skipping...')
    end

    zonesRegistered = true
    
    dbg.debug('RegisterZones called - forcing zone registration for crm-multicharacter compatibility')
    ...
    if Interact.Type == 'ZONE' then
        ...
        if not isResourcePresentProvideless(Config.Interactions) or isResourcePresentProvideless("crm-target") then
            Interact.HandleZone()
            return  
        end
