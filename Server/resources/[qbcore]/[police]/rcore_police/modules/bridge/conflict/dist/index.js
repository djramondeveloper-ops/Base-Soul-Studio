...
var OUTPUT_RESOURCE = "rcore_police";
var OUTPUT_FILE = "scan_results.json";
var IGNORE_RESOURCES = [
  "oxmysql",
  "mysql-async",
  "bob74_ipl",
  "pma-voice",
...
var POLICE_EVENT_PATTERNS = POLICE_EVENT_PREFIXES.map((prefix) => ({
  name: `event: ${prefix}*`,
  regex: new RegExp(`['"\`]${prefix.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")}[\\w:_-]*['"\`]`, "gi")
}));
var EVENT_FUNCTIONS = [
  "RegisterNetEvent",
  "AddEventHandler",
...
var EVENT_KEYWORDS = GENERIC_KEYWORDS;
for (const fn of EVENT_FUNCTIONS) {
  for (const keyword of EVENT_KEYWORDS) {
    POLICE_EVENT_PATTERNS.push({
      name: `fn: ${fn}('*${keyword}*')`,
      regex: new RegExp(`${fn}\\(\\s*['"\`][\\w:.:_-]*${keyword}[\\w:.:_-]*['"\`]`, "gi")
    });
  }
}
async function runScanner() {
  const manifests = await globby([`${RESOURCES_ROOT}/**/fxmanifest.lua`], {
    ignore: [
      "**/node_modules/**",
      ...IGNORE_RESOURCES.map((name) => `**/${name}/**`)
    ]
  });
  const resourceFolders = [];
  for (const file of manifests) {
    const resourceName = import_path.default.basename(import_path.default.dirname(file));
    if (IGNORE_RESOURCES.includes(resourceName)) continue;
    const manifestContent = import_fs.default.readFileSync(file, "utf8");
    if (/this_is_a_map\s*['"]yes['"]/i.test(manifestContent)) continue;
    resourceFolders.push(import_path.default.dirname(file));
  }
  let foundAny = false;
  const results = [];
  for (const folder of resourceFolders) {
    const files = await globby([`${folder}/**/*.lua`], {
      ignore: [
        "**/locales/**",
        "**/locale.lua",
        "**/locales.lua",
    ...
    for (const file of files) {
      ...
      POLICE_EVENT_PATTERNS.forEach((pattern) => {
        let match;
        let regex = new RegExp(pattern.regex, "gi");
        while ((match = regex.exec(content)) !== null) {
          foundAny = true;
          const line = content.substring(0, match.index).split("\n").length;
          const eventMatch = match[0].replace(/['"`]/g, "");
          results.push({
          ...
          console.log(`^6[ConflictScan]^7 ${pattern.name} ^5[${eventMatch}]^7 in ^3${file}^7 (line ${line})`);
        }
      });
    }
  }
  if (!foundAny) {
    console.log("^2[ConflictScan]^7 No police job or cuff/drag/escort events found in valid resources.");
  }
  const rcorePolPath = GetResourcePath(OUTPUT_RESOURCE);
  const outputPath = import_path.default.join(rcorePolPath, OUTPUT_FILE);
  try {
    import_fs.default.writeFileSync(outputPath, JSON.stringify(results, null, 2), "utf8");
    console.log(`
^5[ConflictScan]^7 Results written to ^4${outputPath}`);
    console.log(`
^5[ConflictScan]^7 If you have any issues with escort / other specific parts try to disable those find resources, it might help resolve the issues.`);
  } catch (err) {
    console.error("Error writing scan results:", err);
  }
}
AddEventHandler("rcore_police:server:internal:startScanner", async () => {
  const invoker = GetInvokingResource();
  if (invoker !== "rcore_police") {
    console.log(`^1[ConflictScan]^7 Access denied for ${invoker}! Only rcore_police can trigger the scan.`);
    return;
  }
  console.log(`^5[ConflictScan]^7 [DEBUG] Scanner authorized: resource=${invoker}`);
  await runScanner();
});
