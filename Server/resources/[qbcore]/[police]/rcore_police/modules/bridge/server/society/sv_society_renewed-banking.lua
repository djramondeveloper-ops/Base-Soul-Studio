...
CreateThread(function()
    if Config.Society == Society.RENEWED or Config.Society == Society.QBX_MANAGEMENT then
        SocietyService.GetMoney = function(business)
            if doesExportExistInResource(Society.RENEWED, "getAccountMoney") then
                return safeNumber(exports[Society.RENEWED]:getAccountMoney(business), 0)
            end
            return 0
        end
        SocietyService.RemoveMoney = function(business, amount)
            if doesExportExistInResource(Society.RENEWED, "removeAccountMoney") then
                return exports[Society.RENEWED]:removeAccountMoney(business, amount)
            end
            return false
        end
        SocietyService.AddMoney = function(business, amount)
            if doesExportExistInResource(Society.RENEWED, "addAccountMoney") then
                return exports[Society.RENEWED]:addAccountMoney(business, amount)
            end
            return false
        end
