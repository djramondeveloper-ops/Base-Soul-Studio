...
CreateThread(function()
    if Config.Framework == Framework.QBCore then
        ...
        function Framework.SetPlayerJob(target, jobName, gradeIndex)
            ...
            if gradeIndex == nil then
                gradeIndex = 0 -- Seoul/QBCore: grade 0 = cargo inicial (Cadete/Estagiário)
            end
            local changed = player.Functions.SetJob(jobName, gradeIndex)
            if changed then
                local updatedJob = Framework.getJob(target)
                if updatedJob then
                    TriggerEvent('rcore_police:server:jobUpdate', target, updatedJob)
                end
            end
            return changed
        end
