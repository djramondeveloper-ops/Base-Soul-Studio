-- =====================================================
--  rcore_police · modules/base/client/lib/props/cl-props-barricade.lua
--  Engineered by Eazy Fxap
--  Original: 188 lines → Cleaned: 60 lines
-- =====================================================

function Props.RequestBarricade(model)
    local ped = PlayerPedId()
    local coords = GetEntityCoords(ped)
    local modelData = Config.Props.ModelDataByPropType[PROP_TYPES.BARRICADE]
    ...
    ObjectPlacer.startPlace(model, function(pos, entity, heading)
        ...
        if Config.Props.PG.EnableWhenDeploying then
            CancellableProgress(Config.Props.PG.Time * 1000, _U("PROPS.PLACING_OBJECT", modelData.label or ""), "amb@prop_human_bum_bin@idle_b", "idle_d", 1, function()
                TriggerServerEvent("rcore_police:server:deployProp", { heading = heading, pos = pos, type = PROP_TYPES.BARRICADE })
