...
└── fxmanifest.lua
```
## Revisao Seoul Base - 2026-08-27

Framework real desta instalacao: Creative/vRP. O resource `vrp` declara `provide 'qb-core'`, entao o `mri_Qadmin` usa chamadas `QBCore.Functions.*` por compatibilidade, mas a fonte real de players, grupos, permissoes e inventario continua sendo a base Seoul/vRP.

Dependencias reais usadas nesta base:
...
- O resource possui `database.sql`.
- O `server/db.lua` executa a estrutura do QAdmin no start.
- Esta correcao nao criou tabela nova.

Itens:

- O painel usa os itens existentes da base e do inventario.
- Nenhum item novo foi criado nesta correcao.

Permissoes/grupos:

- Acesso ao painel: grupo vRP/Seoul `Admin`, via `server/seoul_admin_bridge.lua`.
- Permissoes internas continuam no padrao `qadmin.*`.

Arquivos alterados nesta revisao:

- `Server/resources/[core]/vrp/multiframework/server/qbcore.lua`
- `Server/resources/[qbcore]/mri_Qadmin/shared/config.lua`
- `Server/resources/[qbcore]/mri_Qadmin/server/utils.lua`
- `Server/resources/[qbcore]/mri_Qadmin/server/inventory.lua`
- `Server/resources/[qbcore]/mri_Qadmin/server/inventory_callback.lua`
- `Server/resources/[seoul]/Controller/Player/server.lua`
- `Server/resources/[seoul]/Controller/Survival/server.lua`
- `Server/resources/[seoul]/Controller/Tencode/server.lua`
- `Server/resources/[qbcore]/mri_Qadmin/README.md`

O que foi corrigido:

- `QBCore.Functions.Notify` agora ignora `source` invalido antes de chamar `TriggerClientEvent`, evitando o erro `Argument at index 1 was null`.
- `Notify` do QAdmin agora valida `src` antes de disparar notificacao interna.
- Acoes de inventario do QAdmin validam admin, alvo, stash e placa antes de chamar o inventario.
- Callbacks de inventario aceitam `inventory.items` vazio ou ausente sem quebrar em `pairs(nil)`.
- Chamadas de servico do `Controller` usam `{}` quando `vRP.getUsersByPermission` nao retorna tabela.

Como testar:

1. Reinicie `vrp`, `Controller`, `ox_inventory` e `mri_Qadmin`, ou reinicie o servidor.
2. Entre com um personagem no grupo `Admin`.
3. Abra o painel pelo fluxo atual da Seoul.
4. Teste abrir inventario de outro jogador online.
5. Teste dar item com jogador, item e quantidade validos.
6. Dispare arma sem policia online para validar que o `Controller/Player/server.lua` nao mostra `bad argument #1 to 'for iterator'`.
7. Confira no console se nao aparece `Argument at index 1 was null` vindo de `vrp`/`mri_Qadmin`.
