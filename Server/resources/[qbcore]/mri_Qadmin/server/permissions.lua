    ...
    for _, p in ipairs(dbPrincipals) do
        ...
        if string.find(parent, 'group%.') then                       -- '.' escapado
            ...
            if string.find(child, 'license:', 1, true) or string.find(child, 'license2:', 1, true) then
                ...
                local qbplayers = MySQL.query.await('SELECT id FROM characters WHERE License = ?', {cleanLicense}) or {}
                for _, row in ipairs(qbplayers) do
                    MySQL.insert.await('INSERT IGNORE INTO mri_qadmin_character_groups (citizenid, group_id) VALUES (?, ?)', {tostring(row.id), groups[parent]})
                end
...
-- Handshake Seoul: pode ser chamado no login, resource restart ou abertura pelo tablet.
-- A confirmação ao client só ocorre depois que SetupPlayerPrincipals terminou.
RegisterNetEvent('mri_Qadmin:server:PreparePanel', function()
    local src = source
    SetupPlayerPrincipals(src, true)
    TriggerEvent('mri_Qadmin:server:PlayerPermissionsReady', src)
    TriggerClientEvent('mri_Qadmin:client:PanelPermissionsReady', src)
end)
...
lib.callback.register('mri_Qadmin:callback:GetCharacterGroups', function(source, citizenid)
    ...
    local list = {}
    for groupName in pairs(current) do list[#list + 1] = groupName end
    table.sort(list)
    return list
end)
...
lib.callback.register('mri_Qadmin:server:UpdateCharacterGroups', function(source, citizenid, groupsArray)
    ...
    local ok, err = pcall(function()
        -- Mantém a tabela interna do mri para compatibilidade da própria tela de permissões...
        local oldGroups = MySQL.query.await('SELECT group_id FROM mri_qadmin_character_groups WHERE citizenid = ?', {citizenid}) or {}
        ...
        -- ...mas a fonte real da Seoul é vRP permissions/entitydata. Aqui sincronizamos de verdade.
        SeoulQAdminRuntime.SetCharacterGroups(passport, groupsArray)
    end)

    if not ok then
        print('^1[mri_Qadmin] ERRO ao atualizar grupos Seoul do personagem:^7', err)
        return false, "Erro ao salvar grupos da Seoul."
    end

    for _, id in ipairs(QBCore.Functions.GetPlayers()) do
        local p = QBCore.Functions.GetPlayer(id)
        if p and tostring(p.PlayerData.citizenid) == citizenid then
            SetupPlayerPrincipals(id, true)
            if p.Functions and p.Functions.UpdatePlayerData then p.Functions.UpdatePlayerData() end
            break
        end
    end

    TriggerClientEvent('QBCore:Notify', source, 'Grupos reais da Seoul atualizados.', 'success')
    AddLog(source, 'mri_Qadmin', 'permissions', 'warn', ('Grupos Seoul: passaporte %s atualizado: %s'):format(citizenid, table.concat(groupsArray, ', ')), { citizenid = citizenid, groups = groupsArray })
    BroadcastPermissionUpdate()
    return true
end)
