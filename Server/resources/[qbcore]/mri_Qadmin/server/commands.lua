...
local function openSeoulAdminCommand(source)
    if not CheckPerms(source, 'qadmin.open') then return end
    TriggerClientEvent('mri_Qadmin:client:OpenUI', source)
end

lib.addCommand('adm', {
    help = 'Abrir Seoul Admin',
}, openSeoulAdminCommand)

lib.addCommand('seouladmin', {
    help = 'Abrir Seoul Admin fora do tablet',
}, openSeoulAdminCommand)

lib.addCommand('adminpainel', {
    help = 'Abrir Seoul Admin fora do tablet',
}, openSeoulAdminCommand)
