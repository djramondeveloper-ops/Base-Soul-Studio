...
function IsValidPlayerSource(source)
    source = tonumber(source)
    return source ~= nil and source > 0 and GetPlayerName(source) ~= nil
end
...
function HasPerms(source, perms)
    ...
    -- Seoul Base: o acesso real do QAdmin vem somente do grupo vRP/Seoul Admin.
    -- Admin recebe todas as permissões do painel; qualquer outro grupo é bloqueado no server.
    if SeoulQAdminStrictAdminGate then
        if tonumber(source or 0) == 0 then return true end
        return SeoulQAdminHasAdminGroup and SeoulQAdminHasAdminGroup(source) or false
    end
...
--- Extract value from selectedData safely.
--- Suporta dropdown do MRI ({ value = x }), payload direto, quick actions de player
--- ({ id/source/targetId/playerId/citizenid }) e variações de maiúscula/minúscula.
local function unwrapSelectedValue(value)
    if type(value) ~= "table" then return value end
    if value.value ~= nil then return value.value end
    if value.id ~= nil then return value.id end
    if value.source ~= nil then return value.source end
    if value.player ~= nil then return value.player end
    if value.playerId ~= nil then return value.playerId end
    if value.target ~= nil then return value.target end
    if value.targetId ~= nil then return value.targetId end
    if value.citizenid ~= nil then return value.citizenid end
    if value.cid ~= nil then return value.cid end
    if value.name ~= nil then return value.name end
    return nil
end

function GetValue(data, key)
    if data == nil or not key then return nil end

    local wanted = tostring(key):lower()

    -- A NUI do MRI nem sempre manda selectedData como tabela.
    -- Em alguns botões ela envia direto o id/source (number/string).
    -- Antes isso quebrava em pairs(data) ou data[key].
    if type(data) ~= "table" then
        if wanted == "player" or wanted == "jogador" or wanted == "source" or wanted == "id" or wanted == "target" or wanted == "targetid" or wanted == "playerid" then
            return unwrapSelectedValue(data)
        end
        return nil
    end

    if data[key] ~= nil then
        return unwrapSelectedValue(data[key])
    end

    for k, v in pairs(data) do
        if tostring(k):lower() == wanted then
            return unwrapSelectedValue(v)
        end
    end

    if wanted == "player" or wanted == "jogador" then
        return unwrapSelectedValue(data.Player)
            or unwrapSelectedValue(data.player)
            or unwrapSelectedValue(data.source)
            or unwrapSelectedValue(data.src)
            or unwrapSelectedValue(data.id)
            or unwrapSelectedValue(data.target)
            or unwrapSelectedValue(data.targetId)
            or unwrapSelectedValue(data.playerId)
            or unwrapSelectedValue(data.citizenid)
            or unwrapSelectedValue(data.cid)
    end
...
function GetSelectedPlayer(data)
    ...
    local value = GetValue(data, "Player") or GetValue(data, "Jogador")
    return value and tonumber(value) or nil
end

function GetSelectedPassport(data)
    local value = GetValue(data, "Citizen ID") or GetValue(data, "cid") or GetValue(data, "citizenid") or GetValue(data, "Passport") or GetValue(data, "Player")
    return value and tonumber(value) or nil
end
...
function CheckAlreadyPlate(plate)
    ...
    return SeoulQAdminDB.HasVehiclePlate(vPlate)
end
...
function CanTargetPlayer(source, target)
    ...
    if not source or source <= 0 then return false, "admin invalido" end
...
function CheckTargetable(source, target)
    ...
    if not target or target <= 0 then
        if IsValidPlayerSource(source) then
            QBCore.Functions.Notify(source, "Jogador invalido.", "error", 5000)
        end
        return false
    end
