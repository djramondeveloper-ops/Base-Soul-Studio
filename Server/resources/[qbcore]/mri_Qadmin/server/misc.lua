RegisterNetEvent('mri_Qadmin:server:unban_cid', function(_, selectedData)
    ...
    local license = SeoulQAdminDB.GetLicenseByPassport(citizenid)
    if not license then
    ...
    local affectedRows = (SeoulQAdminDB.UnbanLicense(license1) or 0) + (license2 ~= license1 and SeoulQAdminDB.UnbanLicense(license2) or 0)
...
RegisterNetEvent('mri_Qadmin:server:delete_cid', function(_, selectedData)
    ...
    MySQL.query.await('DELETE FROM vehicles WHERE Passport = ?', { tonumber(citizenid) or 0 })
    ...
    if hasWarns then
        local warnLicense = SeoulQAdminDB.GetLicenseByPassport(citizenid)
        if warnLicense then
    ...
    if onlinePlayer and GetResourceState('qbx_core') == 'started' then
        ...
        deleted = SeoulQAdminDB.DeleteCharacter(citizenid)
    end
...
RegisterNetEvent('mri_Qadmin:server:BanPlayer', function(actionKey, selectedData)
    ...
    if targetPlayerOnline then
        -- ONLINE BAN
        SeoulQAdminDB.BanLicense(QBCore.Functions.GetIdentifier(player, 'license'), reason, banTime)
        ...
        if not licenseVal and cid then
             licenseVal = SeoulQAdminDB.GetLicenseByPassport(cid)
        end
        ...
        SeoulQAdminDB.BanLicense(licenseVal, reason, banTime)
...
RegisterNetEvent('mri_Qadmin:server:UnbanPlayer', function(data, selectedData)
    ...
    local result = SeoulQAdminDB.IsLicenseBanned(licenseVal)

    if result then
        SeoulQAdminDB.UnbanLicense(licenseVal)
...
RegisterNetEvent('mri_Qadmin:server:Revive', function(_, selectedData)
    ...
    SeoulQAdminRuntime.Revive(player, 150)
...
RegisterNetEvent('mri_Qadmin:server:ReviveAll', function(_)
    ...
    for _, id in pairs(QBCore.Functions.GetPlayers() or {}) do
        SeoulQAdminRuntime.Revive(tonumber(id), 150)
    end
...
RegisterNetEvent('mri_Qadmin:server:ReviveRadius', function(_)
    ...
    for _, v in pairs(allPlayers) do
        ...
        if dist < 15.0 then
            SeoulQAdminRuntime.Revive(v, 150)
...
RegisterNetEvent('mri_Qadmin:server:ReviveSelf', function()
    ...
    local src = source
    SeoulQAdminRuntime.Revive(src, 150)
...
RegisterNetEvent('mri_Qadmin:server:ToggleBlackout', function(_)
    ...
    Blackout = SeoulQAdminRuntime.SetBlackout(not Blackout)
...
RegisterNetEvent('mri_Qadmin:server:SetSeoulWeather', function(actionKey, selectedData)
    local src = source
    if not CheckPerms(src, 'qadmin.action.change_weather') then return end
    local weather = GetValue(selectedData, 'Weather') or GetValue(selectedData, 'Clima') or actionKey
    local ok, normalized = SeoulQAdminRuntime.SetWeather(weather)
    if not ok then
        return QBCore.Functions.Notify(src, 'Clima inválido: ' .. tostring(weather), 'error', 5000)
    end
    QBCore.Functions.Notify(src, ('Clima alterado para %s.'):format(normalized), 'success', 5000)
    AddLog(src, 'mri_Qadmin', 'server', 'info', ('Clima Seoul alterado para %s'):format(normalized), { weather = normalized })
end)

RegisterNetEvent('mri_Qadmin:server:SetSeoulTime', function(actionKey, selectedData)
    local src = source
    if not CheckPerms(src, 'qadmin.action.change_time') then return end
    local hour = GetValue(selectedData, 'Time Events') or GetValue(selectedData, 'Horário') or GetValue(selectedData, 'Hora') or actionKey
    local ok, h, m = SeoulQAdminRuntime.SetTime(hour, 0)
    if not ok then
        return QBCore.Functions.Notify(src, 'Horário inválido.', 'error', 5000)
    end
    QBCore.Functions.Notify(src, ('Horário alterado para %02d:%02d.'):format(h, m), 'success', 5000)
    AddLog(src, 'mri_Qadmin', 'server', 'info', ('Horário Seoul alterado para %02d:%02d'):format(h, m), { hour = h, minute = m })
end)
...
RegisterNetEvent('mri_Qadmin:server:CuffPlayer', function(_, selectedData)
    ...
    local target = GetSelectedPlayer(selectedData)
...
RegisterNetEvent('mri_Qadmin:server:ClothingMenu', function(_, selectedData)
    ...
    local opened, reason = SeoulQAdminRuntime.OpenClothingFor(src, target)
    if not opened then
        return QBCore.Functions.Notify(src, reason or 'Nenhum sistema de roupas compatível está iniciado.', 'error', 5000)
    end
...
lib.callback.register('mri_Qadmin:callback:GetBans', function(src, data)
    ...
    local bans, total = SeoulQAdminDB.ListBans(search, pageSize, offset)
    return {
        data = bans or {},
        total = total or 0,
        pages = math.ceil((total or 0) / pageSize)
    }
end)
...
RegisterNetEvent('mri_Qadmin:server:unban_rowid', function(_, selectedData)
    ...
    local affectedRows = MySQL.update.await('UPDATE accounts SET Banned = 0, Reason = NULL WHERE id = ?', { banId })
    if affectedRows and affectedRows > 0 then
    ...
    for _, id in ipairs(targetIds) do
        ...
        if ped and ped ~= 0 then
            ...
            if playerObj then
                ...
                local sv = SeoulQAdminRuntime.GetVitalsForSource(id, playerObj.PlayerData.metadata)
                vitals.hunger = sv.hunger
                vitals.thirst = sv.thirst
                vitals.stress = sv.stress
                vitals.isDead = sv.isdead
            end
    ...
    for _, id in pairs(allPlayers) do
        ...
        if ped and ped ~= 0 then
            ...
            if playerObj then
                ...
                vitals.health = GetEntityHealth(ped)
                vitals.armor = GetPedArmour(ped)
                local sv = SeoulQAdminRuntime.GetVitalsForSource(playerSrc, playerObj.PlayerData.metadata)
                vitals.hunger = sv.hunger
                vitals.thirst = sv.thirst
                vitals.stress = sv.stress
                vitals.isDead = sv.isdead
