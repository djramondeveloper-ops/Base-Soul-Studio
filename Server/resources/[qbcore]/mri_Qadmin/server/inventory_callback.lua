...
lib.callback.register('mri_Qadmin:callback:GetPlayerInventory', function(source, targetId)
    ...
    if Player and Player.PlayerData.inventory then
        local items = {}
            for _, item in pairs(Player.PlayerData.inventory or {}) do
        ...
        return {
            items = items,
            weight = 0, -- QBCore nao expoe peso total no PlayerData; UI usa 0 como padrao
...
lib.callback.register('mri_Qadmin:callback:GetVehicleInventory', function(source, plate, invType)
    if not CheckPerms(source, 'qadmin.action.view_inventory') then return nil end

    local invId = invType == 'trunk' and 'trunk'..plate or 'glovebox'..plate
    ...
    if Config.Inventory == 'ox_inventory' then
        ...
        if inventory then
            ...
            return {
                ...
                maxWeight = inventory.maxWeight or 0,
                slots = inventory.slots or 0,
                label = (invType == 'trunk' and 'Porta-malas: ' or 'Porta-luvas: ') .. plate
            }
        end
    end
...
lib.callback.register('mri_Qadmin:server:CopyInventoryToSelf', function(source, targetId, invType)
    ...
    if not invType or invType == 'player' then
        ...
        if TargetPlayer and AdminPlayer and TargetPlayer.PlayerData.inventory then
            for _, item in pairs(TargetPlayer.PlayerData.inventory or {}) do
...
lib.callback.register('mri_Qadmin:server:MoveInventoryItem', function(source, data)
    ...
    Debug('debug', ('Move Request: %s'):format(json.encode(data)))
    ...
    if Config.Inventory == 'ox_inventory' then
        Debug('debug', ('Ox Inventory Move/Swap (Manual): %s, %s, %s, %s, %s'):format(fromTarget, toTarget, data.fromSlot, data.toSlot, data.count))
        ...
        for _, it in pairs(fromInv.items or {}) do
        ...
        for _, it in pairs(toInv.items or {}) do
        ...
        if targetItem then
            ...
            if removedTarget then
                ...
                if not addedSource or not addedTarget then
                    ...
                    Debug('error', 'Swap partial failure! Items might be lost or duplicated in slots.')
                end
            else
        ...
        Debug('debug', 'Ox Manual Move/Swap Success')
        return true
    end
        ...
        if fromPlayer and toPlayer then
            ...
            if itemData then
                if tostring(data.fromId) == tostring(data.toId) then
                    ...
                    Debug('debug', 'QBCore reorder success')
                    return true
                else
                    -- Cross player move
                    if fromPlayer.Functions.RemoveItem(itemData.name, data.count, data.fromSlot) then
                        ...
                        Debug('debug', ('QBCore cross-player move result: %s'):format(added))
                        return added
                    end
                end
            end
...
local function RegisterOxHooks()
    ...
    Debug('debug', 'Registering ox_inventory hooks for real-time sync')
