...
-- Expose current state so DevMode can drive it to an explicit desired state
-- instead of blindly inverting an independent user toggle.
exports('IsLaserActive', function()
    return activeLaser
end)
...
local function LaserLoop()
    ...
    -- Loop ended (toggled off): clear the overlay so it stops spamming.
    SendNUIMessage({
        action = "showEntityInfo",
        data = {
            show = false
...
RegisterNetEvent('mri_Qadmin:client:ToggleLaser', function()
    ...
    -- Only spawn the loop when turning ON and no loop is already running.
    if activeLaser and not wasActive then
        CreateThread(LaserLoop)
    end
end)