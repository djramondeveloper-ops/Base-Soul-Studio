...
local function PlayAlarm(Name,InteriorType)
	...
	TriggerClientEvent("sounds:playSoundDistance",-1,"propertys-alarm-outside-"..Name,"alarm",1.0,false,OutsideCoords,75.0)

	if Interior and Internal[Interior] and Internal[Interior].Exit then
		local InsideCoords = Internal[Interior].Exit
		TriggerClientEvent("sounds:playSoundDistance",-1,"propertys-alarm-inside-"..Name,"alarm",1.0,false,InsideCoords,125.0)
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROPERTYS:ROBBERY
...
AddEventHandler("propertys:Robbery",function(Name)
	...
	if Warehouse then
		TriggerClientEvent("Notify",source,"Aviso","Galpões não podem ser invadidos.","amarelo",5000)
		Active[Passport] = nil
		return false
	end

	if not Service and not Lockpick then
		TriggerClientEvent("Notify",source,"Aviso","Você precisa de uma <b>Lockpick</b> para invadir a propriedade.","amarelo",5000)
		Active[Passport] = nil
		return false
	end
	...
	local TaskResult = Service or (Lockpick and vRP.Task(source,5,2500))
	...
	if TaskResult then
		...
		if not Service and Lockpick then
			TriggerClientEvent("Notify",source,"Aviso","Você falhou ao forçar a fechadura.","vermelho",5000)
		end
	end
...
AddEventHandler("propertys:RobberyItem",function(Number,Name)
	...
	if not Lockpick then
		TriggerClientEvent("Notify",source,"Aviso","Você precisa de uma <yellow>Lockpick</yellow> para roubar.","amarelo",5000)
		return false
	end
	...
	local TaskResult = (Locker and vRP.Safecrack(source,6)) or (not Locker and vRP.Task(source,5,2500))
...
function Creative.Police(Outside,Inside)
	...
	if InsideName[Passport] then
		PlayAlarm(InsideName[Passport],Saved[InsideName[Passport]])
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROPERTYS
...
function Creative.Toggle(Name,Mode)
	...
	if Mode == "Exit" then
		InsideName[Passport] = nil
		Inside[Passport] = nil
		exports.vrp:Bucket(source,"Exit")
		TriggerEvent("vRP:ReloadWeapons",source)
	else
		if Name == "Hotel" and vRP.Scalar("propertys/Count",{ Passport = Passport }) > 0 then
			TriggerClientEvent("Notify",source,"Hotel","Você não possui acesso ao hotel pois já possui propriedade(s).","amarelo",10000)
			return false
		end

		TriggerEvent("DebugWeapons",Passport)
		Inside[Passport] = Propertys[Name].Coords
		InsideName[Passport] = Name
...
AddEventHandler("propertys:Buy",function(Name)
	...
	local HasAccess = vRP.Scalar("propertys/Count",{ Passport = Passport }) <= 0
	TriggerClientEvent("propertys:HotelAccess",source,HasAccess)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PROPERTYS:LOCK
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- ADJUSTITEMFORFRIDGE
-----------------------------------------------------------------------------------------------------------------------------------------
local function AdjustItemForFridge(Item)
	if not ItemDurability(Item) then
		return Item
	end

	local Split = splitString(Item,"-")
	local ItemName = Split[1]
	local OriginalTime = tonumber(Split[2])
	...
	local ElapsedTime = CurrentTime - OriginalTime
	local NewTime = CurrentTime - (ElapsedTime / 2)

	if NewTime < 0 then
		NewTime = CurrentTime
	end

	return ItemName.."-"..math.floor(NewTime)
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- ADJUSTITEMFROMFRIDGE
-----------------------------------------------------------------------------------------------------------------------------------------
local function AdjustItemFromFridge(Item)
	if not ItemDurability(Item) then
		return Item
	end
	...
	local Split = splitString(Item,"-")
	local ItemName = Split[1]
	local FridgeTime = tonumber(Split[2])

	if not FridgeTime then
		return Item
	end

	local CurrentTime = os.time()
	local TimeInFridge = CurrentTime - FridgeTime
	local RealElapsedTime = TimeInFridge * 2
	local NewTime = CurrentTime - RealElapsedTime
...
function Creative.Store(Item,Slot,Amount,Target,Name,Mode)
	...
	if Mode == "Fridge" and ItemDurability(Item) then
		local AdjustedItem = AdjustItemForFridge(Item)
		if AdjustedItem ~= Item then
			local Inv = vRP.Inventory(Passport)
			if Inv[Slot] and Inv[Slot].item == Item then
				Inv[Slot].item = AdjustedItem
				Item = AdjustedItem
			end
		end
	end
...
function Creative.Take(Slot,Amount,Target,Name,Mode)
	...
	local ChestData = (Name == "Hotel") and Mode..":Hotel:"..Passport or Mode..":"..Name
	local Chest = vRP.GetSrvData(ChestData,true)
	
	local ItemToAdjust = nil
	if Mode == "Fridge" and Chest[Slot] and ItemDurability(Chest[Slot].item) then
		ItemToAdjust = Chest[Slot].item
	end

	if vRP.TakeChest(Passport,ChestData,Amount,Slot,Target,true) then
		if Mode == "Fridge" and ItemToAdjust then
			local AdjustedItem = AdjustItemFromFridge(ItemToAdjust)
			local Inv = vRP.Inventory(Passport)
			
			if AdjustedItem ~= ItemToAdjust and Inv[Target] then
				if Inv[Target].item == ItemToAdjust then
					Inv[Target].item = AdjustedItem
				end
			end
		end
...
function Creative.MyPropertys()
	...
	local MyPropertys = {}
	local Result = vRP.Query("propertys/AllUser",{ Passport = Passport })
...
-- CHECKHOTELACCESS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creative.CheckHotelAccess()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		local HasAccess = vRP.Scalar("propertys/Count",{ Passport = Passport }) <= 0
...
-----------------------------------------------------------------------------------------------------------------------------------------
-- SEOUL OX PROPERTY STASH BRIDGE
-----------------------------------------------------------------------------------------------------------------------------------------
local function OxEnabled()
	return GetConvar("seoul:useOxInventory","true") == "true" and GetResourceState("ox_inventory") == "started"
end

local function OxSafeName(Name)
	return tostring(Name or "unknown"):gsub("[^%w:_%-]","_")
end

function Creative.OpenOxStash(Name,Mode)
	local source = source
	local Passport = vRP.Passport(source)
	if not Passport or not Name or not Mode or not OxEnabled() then return false end
	if not Creative.Permission(Name) then return false end

	local Weight = 25
	local DataName = Name
	if Name == "Hotel" then
		DataName = "Hotel:"..Passport
	else
		local Consult = vRP.SingleQuery("propertys/Exist",{ Name = Name })
		if Consult and Consult[Mode] then
			Weight = Consult[Mode]
	...
	local StashId = "seoul_property:"..OxSafeName(Mode..":"..DataName)
	local Label = Mode == "Fridge" and "Geladeira" or "Cofre"
	exports.ox_inventory:RegisterStash(StashId,Label,100,parseInt(Weight) * 1000,nil,nil,nil)
	return StashId
end
