...
-- PLANTS:BURN
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("plants:Burn")
AddEventHandler("plants:Burn", function(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if not Passport then return end

	if not Plants[Number] then
		return
	end

	if vRP.ConsultItem(Passport,"lighter",1) then
		if vRP.Request(source,"Plantação","Deseja realmente <b>Queimar</b> esta planta?") then
			local Coords = Plants[Number].Coords

			Plants[Number] = nil

			TriggerClientEvent("plants:Remove",-1,Number)
			TriggerClientEvent("plants:StartFire",-1,Coords)

			TriggerClientEvent("Notify",source,"Atenção","Você queimou a plantação.","amarelo",5000)
		end
	else
...
AddEventHandler("plants:Destroy",function(Number)
	...
	if Passport and not Active[Passport] and Plants[Number] then
		...
		if vRP.Request(source,"Plantação","Deseja realmente <b>Destruir</b> esta planta?") then
			TriggerClientEvent("Notify",source,"Sucesso","Planta destruída.","verde",5000)
			TriggerClientEvent("plants:Remove",-1,Number)
			Plants[Number] = nil
		end
	end
