			...
			if InVehicle then
				...
				if GetPedInVehicleSeat(Vehicle,-1) == Ped then
					...
					for Number,v in pairs(Spike) do
						if #(VehicleCoords - v["Coords"]) <= 10 then
				...
				if not IsPedOnAnyBike(Ped) and not IsPedInAnyHeli(Ped) and not IsPedInAnyBoat(Ped) and not IsPedInAnyPlane(Ped) then
					...
					if Speed ~= SeatbeltSpeed then
						if not Entity(Vehicle)["state"]["Seatbelt"] and not SeatbeltLock and (SeatbeltSpeed - Speed) >= 50.0 then
							SmashVehicleWindow(Vehicle,math.random(6))
	...
	if IsPedInAnyVehicle(Ped) and not IsPedOnAnyBike(Ped) and not IsPedInAnyHeli(Ped) and not IsPedInAnyBoat(Ped) and not IsPedInAnyPlane(Ped) then
		if SeatbeltLock then
			...
			if Entity(Vehicle)["state"]["Seatbelt"] then
				TriggerEvent("Notify","Atenção","Você removeu o <b>Cinto de Segurança</b>.","amarelo",5000,false,false,false,true)
			end
		else
			...
			local Vehicle = GetVehiclePedIsUsing(Ped)
			if Entity(Vehicle)["state"]["Seatbelt"] then
				TriggerEvent("Notify","Sucesso","Você colocou o <b>Cinto de Segurança</b>.","verde",5000,false,false,false,true)
			end
		end
	end
end)
