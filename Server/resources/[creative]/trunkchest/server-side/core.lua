...
function Creative.OpenOxTrunk()
	local source = source
	local Passport = vRP.Passport(source)
	if not Passport or not Open[Passport] or not OxEnabled() then return false end

	local Data = Open[Passport]
	local StashId = "seoul_trunk:"..OxSafeName(Data.Data or (Data.Passport..":"..Data.Model))
	local Label = "Porta-Malas"
	local Slots = 50
	local Weight = parseInt(Data.Weight or VehicleWeight(Data.Model) or 50) * 1000
