...
function Creative.PenalCode(Mode)
  if Mode == "Arrest" then
    local Articles = exports.oxmysql:query_async(
    "SELECT id AS Id, id AS Value, Section, Article, Contravention, Fine, Arrest, Bail, `Order` FROM mdt_creative_penalcode_articles")
    return Articles
  elseif Mode == "Fine" then
    local Articles = exports.oxmysql:query_async(
    "SELECT id AS Id, id AS Value, Article, Contravention, Fine, Bail, Arrest, CONCAT(Article, ' - ', Contravention) AS Label FROM mdt_creative_penalcode_articles WHERE Fine > 0")
    return Articles
  else
    local Data = {}
    local Sections = exports.oxmysql:query_async(
    "SELECT id, `Order`, Type, Description, Title FROM mdt_creative_penalcode_sections")
    for _, Section in ipairs(Sections) do
      local Articles = exports.oxmysql:query_async(
      "SELECT id AS Id, Article, Contravention, Fine, `Order`, Bail, Arrest FROM mdt_creative_penalcode_articles WHERE Section = ?",
        { Section.id })

      Data[tostring(Section.id)] = { Order = Section.Order, Infractions = Articles, Id = Section.id, Type = Section.Type, Description =
      Section.Description, Title = Section.Title }
    end
    return Data
  end
end
...
function Creative.CreatePenalCode(Mode, Data)
  if Mode == "Section" then
    ...
    "INSERT INTO mdt_creative_penalcode_sections (Title, Description, Type, `Order`) VALUES (?, ?, ?, ?)",
      { Data.Title, Data.Description, Data.Type, Order })
  elseif Mode == "Article" then
    local Order = 0
    local Consult = exports["oxmysql"]:query_async("SELECT * FROM mdt_creative_penalcode_articles WHERE Section = ?",
      { Data.Section })
    if Consult and #Consult > 0 then
      for k, v in pairs(Consult) do
        Order = v.Order + 1
    ...
    exports.oxmysql:insert_async(
    "INSERT INTO mdt_creative_penalcode_articles (Section, Article, Contravention, Fine, Bail, Arrest, `Order`) VALUES (?, ?, ?, ?, ?, ?, ?)",
      { Data.Section, Data.Article, Data.Contravention, Data.Fine, Data.Bail, Data.Arrest or false, Order })
  end
  return true
end
...
function Creative.UpdatePenalCode(Id, Mode, Data)
  ...
  if Mode == "Section" then
    ...
    "UPDATE mdt_creative_penalcode_sections SET Title = ?, Description = ?, Type = ? WHERE id = ?",
      { Data.Title, Data.Description, Data.Type, Id })
  elseif Mode == "Article" then
    exports.oxmysql:execute_async(
    "UPDATE mdt_creative_penalcode_articles SET Article = ?, Contravention = ?, Fine = ?, Bail = ?, Arrest = ? WHERE id = ?",
      { Data.Article, Data.Contravention, Data.Fine, Data.Bail, Data.Arrest, Id })
...
function Creative.OrderPenalCode(Identifier, Mode, Direction, Section)
  ...
  "UPDATE " ..
  TableName ..
  " SET `Order` = CASE WHEN id = @Id THEN @TargetOrder WHEN id = @NeighborId THEN @CurrentOrder END WHERE id IN (@Id, @NeighborId)",
    { Id = Identifier, NeighborId = Neighbor.id, CurrentOrder = CurrentOrder, TargetOrder = TargetOrder })
  ...
  for Category, CategoryInfo in pairs(Permissions) do
    if type(CategoryInfo) == "table" then
      ...
      for SubCategory, Level in pairs(Permissions[Category]) do
        PermissionsResult[Category][SubCategory] = Level > 0 and Level >= Hierarchy or Level < 0 and false or
        Level == 0 and true
      end
    else
      PermissionsResult[Category] = CategoryInfo > 0 and CategoryInfo >= Hierarchy or CategoryInfo < 0 and false or
      CategoryInfo == 0 and true
    end
  end
...
function Creative.Home()
  ...
  local Consult = exports["oxmysql"]:query_async("SELECT * FROM mdt_creative_board WHERE Permission = @Permission",
  ...
  for Level, _ in pairs(Hierarchy) do
    Divisions[#Divisions + 1] = { Amount = vRP.AmountService(Permission, Level), Name = vRP.NameHierarchy(Permission,
      Level) }
  end
...
function Creative.UpdateBoard(Title, Description)
  ...
  for _, Target in pairs(Groups) do
    if Target ~= source then
      TriggerClientEvent("Notify", Target, Name, "<b class=\"text-white\">" .. Title .. "</b>: " .. Description,
        "amarelo")
    end
  end
...
function Creative.User(Passport)
  ...
  for k, v in ipairs(Consult) do
    ...
    Records[#Records + 1] = { Id = v.id, Type = v.Type, Date = v.Timestamp, Officer = v.Officer, Fine = v.Fine, Arrest =
    v.Arrest, Description = v.Description, Infractions = v.Infractions, Paid = v.Paid, Hour = v.Hour }
  end

  local Wanted = exports.oxmysql:scalar_async([[ SELECT COUNT(*) FROM mdt_creative_wanted WHERE Passport = ? ]],
    { Passport })
  local Sex = (vRP.Identity(Passport).Skin == "mp_f_freemode_01") and "F" or "M"

  return { { Name = vRP.FullName(Passport), Phone = vRP.Phone(Passport), Flyingarms = vRP.DatatableInformation(Passport, "Flyingarms") or false, Firearms = vRP.DatatableInformation(Passport, "Firearms") or false, Gender = Sex, Passport = Passport, Services = vRP.Identity(Passport).Prison, Wanted = Wanted, Fines = Total, Avatar = exports.vrp:Avatar(Passport) },
    Records }
end
...
function Creative.Record(Data)
  local Records = {
    ["fine"] =
    "SELECT id, 'fine' as Type, Timestamp, Officer, Fine, Arrest, Description, Paid, Hour, Infractions FROM mdt_creative_fines WHERE id = ?",
    ["arrest"] =
    "SELECT id, 'arrest' as Type, Timestamp, Officer, Officers, Fine, Arrest, Description, Infractions FROM mdt_creative_arrest WHERE id = ?",
  ...
  if type(Infractions) == "table" and #Infractions > 0 then
    ...
    "SELECT CONCAT(Article, ' - ', Contravention) AS Label FROM mdt_creative_penalcode_articles WHERE id IN (" ..
    table.concat(Placeholders, ",") .. ")", Infractions)
    local Labels = {}
  ...
  return { Id = Result.id, Type = Result.Type, Date = Result.Timestamp, Officer = Result.Officer, Officers = Result
  .Officers, Fine = Result.Fine, Arrest = Result.Arrest, Description = Result.Description, Infractions = Result
  .Infractions, Paid = Result.Paid, Hour = Result.Hour }
end
...
function Creative.Operations()
  ...
  for Index, Data in pairs(Operations) do
    Operation[Index] = { Candidates = Data.Candidates, Location = Data.Location, Radio = Data.Radio, Creator = Data
    .Creator, Escalates = Data.Escalates or { [1] = Data.Creator } }
  end
...
function Creative.EscalatedOperation(Identifier, Mode, Passport)
  if Mode == "Apply" then
    Operations[Identifier].Candidates[#Operations[Identifier].Candidates + 1] = { Passport = Passport, Name = vRP
    .FullName(Passport) }
  elseif Mode == "Add" then
    Operations[Identifier].Escalates[#Operations[Identifier].Escalates + 1] = { Passport = Passport, Name = vRP.FullName(
    Passport) }
...
function Creative.ArrestRecords()
  ...
  for _, v in ipairs(Consult) do
    Records[#Records + 1] = { Id = v.id, Passport = v.Passport, Name = vRP.FullName(v.Passport), Arrest = v.Arrest, Fine =
    v.Fine, Date = v.Timestamp, Officers = v.Officers }
  end
...
function Creative.Arrest(Data)
  ...
  local Articles = exports.oxmysql:query_async(
  ("SELECT Article, `Fine`, `Arrest` FROM `mdt_creative_penalcode_articles` WHERE `id` IN (%s)"):format(string.rep("?,",
    #Data.Infractions):sub(1, -2)), Data.Infractions)
  ...
  local Arrest = exports.oxmysql:insert_async(
  "INSERT INTO `mdt_creative_arrest` (`Passport`, `Officer`, `Officers`, `Timestamp`, `Infractions`, `Arrest`, `Fine`, `Description`) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ",
    { Passport, Officer, json.encode(Data.OfficersInvolved), Timestamp, table.concat(Infractions, ", "), Services, Fine,
      Description })
...
function Creative.Fine(Data)
  ...
  local Articles = exports.oxmysql:query_async(
  ("SELECT `Fine` FROM `mdt_creative_penalcode_articles` WHERE `id` IN (%s)"):format(table.concat(Data.Infractions, ",")
  :gsub("[^,]", "?")), Data.Infractions)
  ...
  exports.oxmysql:insert_async(
  "INSERT INTO `mdt_creative_fines` (`Passport`, `Officer`, `Timestamp`, `Infractions`, `Fine`, `Description`, `Paid`, `Arrest`, `Date`, `Hour`) VALUES (?, ?, ?, ?, ?, ?, 0, NULL, ?, ?)",
    { Passport, Officer, Timestamp, json.encode(Data.Infractions), Fine, Description or "", Date, Hour })
...
function Creative.Warning(Data)
  ...
  local Consult = exports.oxmysql:execute_async(
  "INSERT INTO mdt_creative_warning (Passport, Officer, Timestamp, Description) VALUES (?, ?, ?, ?)",
    { Target, Passport, Timestamp, Description })
  if Consult then
...
function Creative.PoliceReports()
  ...
  for k, v in pairs(Consult) do
    Reports[#Reports + 1] = { Creator = { Name = vRP.FullName(v.Officer) }, Date = v.Timestamp, Id = v.id, Applicant =
    vRP.FullName(v.Officer), Archive = v.Archive, Title = v.Title }
  end
...
function Creative.GetPoliceReport(Id)
  ...
  for k, v in pairs(Consult) do
    ...
    Reports[#Reports + 1] = { Date = v.Timestamp, Description = v.Description, Title = v.Title, Creator = { Name = vRP.FullName(v.Officer), Passport = v.Officer }, Applicant = { Name = vRP.FullName(v.Officer), Passport = v.Officer }, Suspects =
    Suspects }
  end
...
function Creative.CreatePoliceReport(Data)
  ...
  local Reports = exports.oxmysql:insert_async(
  "INSERT INTO mdt_creative_reports (Passport, Title, Suspects, Officer, Timestamp, Description, Archive) VALUES (?, ?, ?, ?, ?, ?, ?)",
    { Passport, Data.Title, json.encode(Suspects), Passport, os.time(), Description, 0 })
...
function Creative.UpdatePoliceReport(Data)
  ...
  local Update = { Title = Data.Title or Existing.Title, Description = Description or Existing.Description, Suspects =
  Data.Suspects and json.encode(Data.Suspects) or Existing.Suspects, Archive = Data.Archive ~= nil and Data.Archive or
  Existing.Archive }

  local Consult = exports.oxmysql:execute_async(
  "UPDATE mdt_creative_reports SET Title = ?, Description = ?, Suspects = ?, Archive = ? WHERE id = ?",
    { Update.Title, Update.Description, Update.Suspects, Update.Archive, Data.Id })
  ...
  if Consult then
    ...
    return { Id = Reports.id, Title = Reports.Title, Description = Reports.Description, Date = Reports.Timestamp, Suspects =
    Formatted, Archive = Reports.Archive, Creator = { Name = vRP.FullName(Reports.Passport), Passport = Reports.Passport } }
  else
...
function Creative.ArchivePoliceReport(Reports, Status)
  ...
  local Consult = exports.oxmysql:execute_async("UPDATE mdt_creative_reports SET Archive = ? WHERE id = ?",
    { Status and 1 or 0, Reports })
...
function Creative.Wanted()
  ...
  for _, v in ipairs(Consult) do
    WantedList[#WantedList + 1] = { Citizen = { Passport = v.Passport, Name = vRP.FullName(v.Passport) }, Id = v.id, Date =
    v.Timestamp, Image = v.Image, Description = v.Description, Officer = v.Officer and
    ("#%i - %s"):format(v.Officer, vRP.FullName(v.Officer)) or "Desconhecido", HowLong = v.HowLong }
  end
...
function Creative.GetWanted(Id)
  ...
  local Wanted = exports.oxmysql:single_async(
  " SELECT id, Passport, Image, Accusations, Officer, Timestamp, HowLong, Description FROM mdt_creative_wanted WHERE id = ? ",
    { Id })
...
function Creative.CreateWanted(Data)
  ...
  local Timestamp = os.time()
  local Result = exports.oxmysql:insert_async(
  "INSERT INTO mdt_creative_wanted (Passport, Image, Accusations, Officer, Timestamp, HowLong, Description) VALUES (?, ?, ?, ?, ?, ?, ?)",
    { Data.Passport or Data.Citizen, Data.Image, json.encode(Data.Accusations), Passport, Timestamp, Data.HowLong,
      Description or "" })

  if Result then
    local NewRecord = { Citizen = { Passport = Data.Passport or Data.Citizen, Name = vRP.FullName(Data.Passport or Data.Citizen) }, Id =
    Result, Date = Timestamp, Image = Data.Image, Description = Description or "", Officer = ("#%i - %s"):format(
    Passport, vRP.FullName(Passport)), HowLong = Data.HowLong }

    TriggerClientEvent("mdt:Notify", source, "Sucesso", "Registro de procurado criado com sucesso.", "verde")
    return NewRecord
  else
    TriggerClientEvent("mdt:Notify", source, "Erro", "Falha ao criar registro de procurado no banco de dados.",
      "vermelho")
    return false
  end
end
...
function Creative.UpdateWanted(Data)
  ...
  local Result = exports.oxmysql:execute_async(
  "UPDATE mdt_creative_wanted SET Description = ?, HowLong = ?, Image = ? WHERE id = ?",
    { Description, Data.HowLong, Data.Image, Data.Id })
  ...
  if Service and OtherPassport then
    ...
    if Vehicle[1] then
      if not Vehicle[1]["Arrest"] then
        TriggerClientEvent("mdt:Vehicle", source, OtherPassport, vRP.FullName(OtherPassport), Plate,
          Vehicle[1]["Vehicle"])
      else
        TriggerClientEvent("Notify", source, "Departamento Policial", "Veículo já se encontra apreendido.", "policia",
          5000)
      end
    end
  end
...
function Creative.InternalAffairs()
  ...
  for k, v in pairs(Consult) do
    Affairs[#Affairs + 1] = { Creator = { Name = vRP.FullName(v.Passport), Passport = v.Passport }, Date = v.Timestamp, Id =
    v.id, Applicant = vRP.FullName(v.Passport), Archive = v.Archive, Title = v.Title, Description = v.Description, Suspect = { Name = vRP.FullName(v.Suspect), Passport = v.Suspect } }
  end
...
function Creative.CreateInternalAffairs(Data)
  ...
  local Consult = exports.oxmysql:insert_async(
  "INSERT INTO mdt_creative_internalaffairs (Passport, Title, Suspect, Officer, Timestamp, Description, Archive) VALUES (?, ?, ?, ?, ?, ?, ?)",
    { Passport, Data.Title, Data.Suspect, Passport, os.time(), Description, 0 })
...
function Creative.UpdateInternalAffairs(Data)
  ...
  local Consult = exports.oxmysql:update_async(
  [[UPDATE mdt_creative_internalaffairs SET Title = ?, Description = ?, Suspect = ?, Officer = ?, Timestamp = ? WHERE id = ?]],
    { Data.Title, Description, Data.Suspect, Data.Officer or Passport, os.time(), Data.Id })
...
function Creative.ArchiveInternalAffairs(Id)
  ...
  local Archive = exports.oxmysql:update_async("UPDATE mdt_creative_internalaffairs SET Archive = ? WHERE id = ?",
    { Affairs, Id })
...
function Creative.CreateSeizedVehicle(Data)
  ...
  "INSERT INTO mdt_creative_vehicles (Passport, Officer, Image, Vehicle, Plate, Location, Timestamp, Description ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
    { Data.Passport, vRP.Passport(source), Data.Image, Data.Vehicle, Data.Plate, Data.Location, os.time(), Description },
    function(Success)
      if Success then
...
function Creative.GetMedal(Id)
  ...
  "SELECT id, Name, Officers, Image FROM mdt_creative_medals WHERE Id = ?", { Id })
  local Officers = {}
...
function Creative.CreateMedal(Data)
  ...
  local Consult = exports["oxmysql"]:insert_async("INSERT INTO mdt_creative_medals (Image, Name) VALUES (@Image, @Name)",
    { Image = Data["Image"], Name = Data["Name"], })

  if Consult then
    TriggerClientEvent("mdt:Notify", source, "Sucesso",
      "A medalha <b class=\"text-white\">" .. Data["Name"] .. "</b> foi criada com sucesso.", "verde")
  else
    TriggerClientEvent("mdt:Notify", source, "Erro",
      "Falha ao criar a medalha <b class=\"text-white\">" .. Data["Name"] .. "</b>.", "vermelho")
    return false
  end
...
function Creative.UpdateMedal(Data)
  ...
  local Consult = exports["oxmysql"]:execute_async("UPDATE mdt_creative_medals SET Name = ?, Image = ? WHERE id = ?",
    { Data["Name"], Data["Image"], Data["Id"] })

  if Consult then
    TriggerClientEvent("mdt:Notify", source, "Sucesso",
      "A medalha <b class=\"text-white\">" .. Data["Name"] .. "</b> foi atualizada com sucesso.", "verde")
    return true
  else
    TriggerClientEvent("mdt:Notify", source, "Erro",
      "Falha ao atualizar a medalha <b class=\"text-white\">" .. Data["Name"] .. "</b>.", "vermelho")
    return false
  end
end
...
function Creative.AssignMedal(Data)
  ...
  local Consult = exports["oxmysql"]:single_async("SELECT Name, Officers FROM mdt_creative_medals WHERE Id = ?",
    { Data["Id"] })
  ...
  for _, Member in ipairs(Officers) do
    if tostring(Member) == tostring(Data["Officer"]) then
      TriggerClientEvent("mdt:Notify", source, "Erro",
        "O passaporte <b class=\"text-white\">" ..
        Data["Officer"] .. "</b> já possui a medalha <b class=\"text-white\">" .. Consult.Name .. "</b>.", "vermelho")
      return false
    end
  end
  ...
  exports["oxmysql"]:execute_async("UPDATE mdt_creative_medals SET Officers = ? WHERE Id = ?",
    { json.encode(Officers), Data["Id"] })
...
function Creative.RemoveMedal(Data)
  ...
  if Sucess then
    TriggerClientEvent("mdt:Notify", source, "Sucesso",
      "O passaporte <b class=\"text-white\">" ..
      Passport .. "</b> foi removido com sucesso da medalha <b class=\"text-white\">" .. Consult.Name .. "</b>.", "verde")
  else
    TriggerClientEvent("mdt:Notify", source, "Erro",
      "Não foi possível localizar o passaporte <b class=\"text-white\">" ..
      Passport .. "</b> na medalha <b class=\"text-white\">" .. Consult.Name .. "</b>", "vermelho")
  end
...
function Creative.GetUnit(Id)
  ...
  if not Config.Permissions.Units.View == Hierarchy then
    TriggerClientEvent("mdt:Notify", source, "Erro", "Você não possui permissões necessárias para visualizar unidades.",
      "vermelho")
    return false
  end

  local Consult = exports.oxmysql:single_async("SELECT * FROM mdt_creative_units WHERE id = ? AND Permission = ?",
    { Id, Permission })
...
function Creative.CreateUnit(Data)
  ...
  if not Config.Permissions.Units.Create == Hierarchy then
    TriggerClientEvent("mdt:Notify", source, "Erro", "Você não possui permissões necessárias para criar unidades.",
      "vermelho")
    return false
  end
  ...
  local Result = exports.oxmysql:insert_async(
  "INSERT INTO mdt_creative_units (Image, Name, Permission, Officers) VALUES (?, ?, ?, ?)",
    { Image, Data.Name, Permission, "[]" })
...
function Creative.UpdateUnit(Data)
  ...
  if not Config.Permissions.Units.Edit == Hierarchy then
    TriggerClientEvent("mdt:Notify", source, "Erro", "Você não possui permissões necessárias para editar unidades.",
      "vermelho")
    return false
  end
  ...
  local Result = exports.oxmysql:execute_async("UPDATE mdt_creative_units SET Name = ?, Image = ? WHERE id = ?",
    { Data.Name, Data.Image or Consult.Image, Data.Id })

  if Result then
    TriggerClientEvent("mdt:Notify", source, "Sucesso", "Unidade <b>" .. Data.Name .. "</b> atualizada com sucesso.",
      "verde")

    return { Name = Data.Name, Image = Data.Image or Consult.Image, Id = Data.Id, Officers = json.decode(Consult
    .Officers) or {} }
  else
...
function Creative.AssignUnit(Data)
  ...
  local Consult = exports.oxmysql:execute_async("UPDATE mdt_creative_units SET Officers = ? WHERE id = ?",
    { json.encode(Officers), Number })

  if Consult then
    local Name = vRP.FullName(Data)
    TriggerClientEvent("mdt:Notify", source, "Sucesso", ("Oficial %s adicionado à unidade %s"):format(Name, Consult.Name),
      "verde")

    local Target = vRP.Source(Data)
    if Target then
      TriggerClientEvent("Notify", Target, "Unidade", ("Você foi designado para a unidade %s"):format(Consult.Name),
        "verde", 10000)
    end
...
function Creative.RemoveUnit(Data)
  ...
  if UnitId <= 0 and type(Data.Id or Data.UnitId) == "string" then
    ...
    "SELECT id FROM mdt_creative_units WHERE Name = ? AND Permission = ? LIMIT 1", { unitName, Perm })
  ...
  local function pushOfficer(v)
    ...
    if not num and type(v) == "table" then
      num = tonumber(v.Value) or tonumber(v.value) or tonumber(v.Passport) or tonumber(v.passport) or tonumber(v.id) or
      tonumber(v.ID)
    end
  ...
  if Hierarchy ~= Config.Permissions.Units.Assign then
    TriggerClientEvent("mdt:Notify", source, "Erro", "Você não tem permissão para remover oficiais da unidade.",
      "vermelho")
    return false
  end
  ...
  local Updated = exports.oxmysql:execute_async(
  "UPDATE mdt_creative_units SET Officers = ? WHERE id = ? AND Permission = ?", { json.encode(Officers), UnitId, Perm })
  if Updated then
    for _, Officer in ipairs(OfficerList) do
      local Name = vRP.FullName(Officer)
      local Target = vRP.Source(Officer)
      if Target then
        TriggerClientEvent("Notify", Target, "Unidade", ("Você foi removido da unidade %s"):format(Consult.Name),
          "amarelo", 10000)
      end

      TriggerClientEvent("mdt:Notify", source, "Sucesso",
        ("Oficial %s removido da unidade %s."):format(Name, Consult.Name), "verde")
    end
...
function Creative.DestroyUnit(Id)
  ...
  if not Config.Permissions.Units.Delete == Hierarchy then
    TriggerClientEvent("mdt:Notify", source, "Erro", "Você não possui permissões necessárias para remover unidades.",
      "vermelho")
    return false
  end
  ...
  if Result then
    TriggerClientEvent("mdt:Notify", source, "Sucesso", "Unidade <b>" .. Consult.Name .. "</b> removida com sucesso.",
      "verde")
    return true
  else
...
function Creative.Officers(Management, Ranking)
  ...
  for Member, _ in pairs(Members) do
    ...
    local Consult = exports.oxmysql:query_async("SELECT * FROM mdt_creative_medals WHERE JSON_CONTAINS(Officers, ?)",
      { tostring(Member) })
    ...
    local Units = {}
    local Consult = exports.oxmysql:query_async(
    "SELECT * FROM mdt_creative_units WHERE JSON_CONTAINS(Officers, ?) AND Permission = ?",
      { tostring(Member), Permission })
    for _, Unit in pairs(Consult) do
      Units[#Units + 1] = { Name = Unit.Name, Image = Unit.Image }
    end

    local Data = { Name = vRP.FullName(Member), Passport = Member, Patent = vRP.HasPermission(Member, Permission), Service =
    Service[tostring(Member)] and 1 or 0, Units = Units, Medals = Medals }
...
function Creative.HierarchyOfficer(Data)
  ...
  if Mode:find("Promote") or Mode:find("Demote") then
    ...
    return { Passport = Target, Name = (Identity["Name"] or "Indivíduo") .. " " .. (Identity["Lastname"] or "Indigente"), Hierarchy =
    vRP.HasPermission(Target, Permission), Service = vRP.Source(Target) and 1 or 0 }
  end
...
function Creative.Bank()
  ...
  local Consult = exports['oxmysql']:query_async(
  'SELECT * FROM painel_creative_transactions WHERE Permission = @Permission LIMIT 50', { Permission = Permission })

  local Transactions = {}
  for _, Data in ipairs(Consult) do
    table.insert(Transactions,
      { Player = { Passport = Data.Passport, Name = vRP.FullName(Data.Passport) }, To = { Passport = Data.Transfer, Name = vRP.FullName(Data.Transfer) }, Type =
      Data.Type, Value = Data.Value, Date = Data.Timestamp })
  end
...
function Creative.DepositBank(Value)
  ...
  if vRP.PaymentBank(Passport, Value) then
    exports.oxmysql:insert_async(
    "INSERT INTO painel_creative_transactions (Type, Passport, Value, Timestamp, Permission) VALUES (\"Deposit\", ?, ?, ?, ?)",
      { Passport, Value, os.time(), Permission })
...
function Creative.WithdrawBank(Value)
  ...
  exports.oxmysql:insert_async(
  "INSERT INTO painel_creative_transactions (Type, Passport, Value, Timestamp, Permission) VALUES (\"Withdraw\", ?, ?, ?, ?)",
    { Passport, Value, os.time(), Permission })
...
function Creative.TransferBank(Target, Value)
  ...
  exports.oxmysql:insert_async(
  "INSERT INTO painel_creative_transactions (Type, Passport, Value, Transfer, Timestamp, Permission) VALUES (\"Transfer\", ?, ?, ?, ?, ?)",
    { Passport, Value, Target, os.time(), Permission })
