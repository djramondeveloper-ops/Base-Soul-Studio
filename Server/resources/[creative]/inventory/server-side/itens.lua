...
-----------------------------------------------------------------------------------------------------------------------------------------
Use = {
	["medicshop"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if Application and Coords and not vCLIENT.ObjectExists(source,Coords,Hash) then
			if vRP.TakeItem(Passport,Full,1,true,Slot) then
				...
				Objects[Selected] = { Coords = Coords, Object = Hash, Item = Full, Mode = "Medic", Weight = 0.75, Bucket = GetPlayerRoutingBucket(source) }
		...
		if not vRP.InsideVehicle(source) then
			...
			if Vehicle then
				if vRP.PassportPlate(Plate) then
					if vRP.Request(source,"Freios","Deseja realmente aplicar <b>"..Dotted(Amount).."x "..ItemName(Item).."</b> no veículo?") then
						...
						for Passport,PlayerSource in pairs(Players) do
							async(function()
								TriggerClientEvent("player:VehicleHood",PlayerSource,Network,"open")
							end)
						end

						if vRP.Task(source,1,5000) then
							Active[Passport] = os.time() + 5 * Amount
							TriggerClientEvent("Progress",source,"Reparando",5000 * Amount)
							...
							CreateThread(function()
								...
								if Active[Passport] then
									...
									if vRP.TakeItem(Passport,Full,Amount,true,Slot) then
										...
										for _,v in pairs(Players) do
											table.insert(PlayersAround, v)
										end
										TriggerEvent("engine:ApplyBrakes",Item,Network,Amount,PlayersAround)
									end
								end
								...
								for Passport,PlayerSource in pairs(Players) do
								async(function()
									TriggerClientEvent("player:VehicleHood",PlayerSource,Network,"close")
								end)
							end
					...
					TriggerClientEvent("Notify",source,"Atenção","Você não pode usar isso em veículos não registrados com emplacamento.","amarelo",5000)
				end
			end
		else
			TriggerClientEvent("Notify",source,"Atenção","Você não pode usar isso dentro de um veículo.","amarelo",5000)
		end
	end,
	...
	["umbrella"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if Player(source)["state"]["Umbrella"] then
			vRPC.Destroy(source)

			Player(source)["state"]["Umbrella"] = false
		else
			TriggerClientEvent("animations:Emotes",source,"chuva","Umbrella")

			Player(source)["state"]["Umbrella"] = true
		end
	end,

	["antipyretic"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if Player(source)["state"]["Illness"] then
			...
			CreateThread(function()
				...
				if Active[Passport] then
					...
					if vRP.TakeItem(Passport,Full,1,true,Slot) then
						TriggerEvent("health:Cure",Item,source)
						vRPC.UpgradeHealth(source,10)
					end
				end
			end)
		else
			TriggerClientEvent("inventory:Notify",source,"Atenção","Você precisa estar doente para utilizar <b>"..ItemName(Item).."</b>.","amarelo")
		end
	end,

	["intoxication"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if Player(source)["state"]["Illness"] then
			Active[Passport] = os.time() + 10
			Player(source)["state"]["Buttons"] = true
			TriggerClientEvent("inventory:Close",source)
	...
	["complaint"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if Player(source)["state"]["Illness"] then
			Active[Passport] = os.time() + 10
			Player(source)["state"]["Buttons"] = true
			TriggerClientEvent("inventory:Close",source)
	...
	["antiinfection"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if Player(source)["state"]["Illness"] then
			Active[Passport] = os.time() + 10
			Player(source)["state"]["Buttons"] = true
			TriggerClientEvent("inventory:Close",source)
	...
	["medkit"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		Player(source)["state"]["Buttons"] = true
		TriggerClientEvent("inventory:Close",source)
		TriggerClientEvent("Progress",source,"Usando",25000)
		vRPC.playAnim(source,true,{"amb@world_human_clipboard@male@idle_a","idle_c"},true)
	...
	["joint"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if vRP.ConsultItem(Passport,"lighter",1) then
			...
			TriggerClientEvent("Notify",source,"Atenção","Você precisa de <b>1x "..ItemName("lighter").."</b>.","amarelo",5000)
		end
	end,
	...
	["lockpick"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle then
				...
				if Entity(Networked)["state"]["Lockpick"] then
					TriggerClientEvent("Notify",source,"Aviso","O miolo da ignição deste veículo já foi danificado.","vermelho",5000)
					return false
				end
				...
				if vRP.InsideVehicle(source) then
					...
					if vRP.Task(source,3,2000) then
						...
						if DoesEntityExist(Networked) then
							if not vRP.PassportPlate(Plate) then
								...
								TriggerClientEvent("inventory:StartVehicleAlarm",-1,Network)

								SetVehicleDoorsLocked(Networked,1)
								Entity(Networked)["state"]:set("Lockpick",Plate,true)
								TriggerClientEvent("Notify",source,"Sucesso","O miolo da ignição deste veículo foi danificado.","verde",5000)
					...
					if vRP.Task(source,6,2000) then
						Active[Passport] = os.time() + 15
						vGARAGE.RegisterDecors(source,Vehicle)
	...
	["moneywash"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		if Application and Coords and not vCLIENT.ObjectExists(source,Coords,Hash,0.675) and vRP.TakeItem(Passport,Full,1,true,Slot) then
			exports.moneywash:Wash(Passport,Full,Hash,Coords,Route,Dirty,Cleanup,"wetdollar","dollar")
		end
	...
	["basket"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		TriggerClientEvent("inventory:Basket",source)
	end,
		...
		if not vRP.InsideVehicle(source) then
			...
			if ClosestPed and not vRP.IsEntityVisible(ClosestPed) then
				...
				if Player(ClosestPed)["state"]["Handcuff"] then
					...
					TriggerClientEvent("sounds:playSound",source,"uncuff","uncuff",0.5,false)
					TriggerClientEvent("sounds:playSound",ClosestPed,"uncuff","uncuff",0.5,false)
					...
					if vRP.GetHealth(ClosestPed) > 100 then
						...
						SetTimeout(3500,function()
							...
							TriggerClientEvent("sounds:playSound",source,"cuff","cuff",0.5,false)
							TriggerClientEvent("sounds:playSound",ClosestPed,"cuff","cuff",0.5,false)
		...
		if Application and Coords then
			...
			SetTimeout(5000,function()
				if Active[Passport] then
					Active[Passport] = os.time() + 10
					TriggerClientEvent("Progress",source,"Colocando",10000)
					TriggerClientEvent("sounds:playSound",source,"sprays","sprays",0.5,false)
...
for _,v in ipairs(Clones) do
	for _,w in ipairs(Puritys) do
		Use[v.Clone.."clone_"..w.Percent] = function(source,Passport,Amount,Slot,Full,Item,Split)
			if v.Clone == "weed" then
				if not vRP.ConsultItem(Passport,"bucket",1) then
					TriggerClientEvent("Notify",source,"Atenção","Você precisa de <b>1x "..ItemName("bucket").."</b>.","amarelo",5000)
					return
				end
			end
			...
			if Application and Coords and not vCLIENT.ObjectExists(source,Coords,Hash) then
				if vRP.TakeItem(Passport,Full,1,true,Slot) then
					...
					if v.Clone == "weed" then
						vRP.TakeItem(Passport,"bucket",1,true)
					end
				end
			end
