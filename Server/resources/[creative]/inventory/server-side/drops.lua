...
function Creative.Drops(Item,Slot,Amount)
	...
	if vRP.TakeItem(Passport,Item,Quantity,false,Slot) then
		local Result = exports.inventory:Drops(Passport,source,Item,Quantity,true)
		TriggerClientEvent("inventory:Update",source)
		return Result
	end
		...
		if not Provisory.desc then
			if Item == "vehiclekey" and Split[2] then
				...
				if Consult and VehicleExist(Consult.Vehicle) then
					Provisory.desc = "Proprietário: <common>"..vRP.FullName(Consult.Passport).."</common><br>Modelo: <common>"..VehicleName(Consult.Vehicle).."</common><br>Placa: <common>"..Split[2].."</common>"
				else
					Provisory.desc = "Proprietário: <epic>Prefeitura</epic><br>Placa: <common>"..Split[2].."</common>"
				end
			...
			elseif ItemNamed(Item) and Split[2] and vRP.Identity(Split[2]) then
				if Item == "identity" then
					Provisory.desc = "Passaporte: <rare>"..Dotted(Split[2]).."</rare><br>Nome: <rare>"..vRP.FullName(Split[2]).."</rare>"
				else
