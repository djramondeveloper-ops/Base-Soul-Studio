...
function Open(Vehicle,Logo)
	...
	Information["Model"] = string.lower(GetEntityArchetypeName(Information["Vehicle"]))
...
RegisterNUICallback("Apply",function(Data,Callback)
	...
	SendNUIMessage({ Action = "Price", Payload = Calculate(Initial,string.lower(vRP.VehicleName())) })
				...
				for Index, v in pairs(Locations) do
					...
					if Distance <= 6.5 then
						...
						if Distance <= 1.25 then
							SetDrawOrigin(v["Coords"]["xyz"].x, v["Coords"]["xyz"].y, v["Coords"]["xyz"].z)
							DrawSprite("Textures", "EPress", 0.0, 0.0, 0.053, 0.01 * GetAspectRatio(false), 0.0, 255, 255, 255, 255)
							ClearDrawOrigin()

							if IsControlJustPressed(1,38) then
								local Model,Network,Plate = vRP.VehicleName()
								if not Model or not Plate then
									TriggerEvent("Notify","Negado","Este veículo não está registrado no banco de dados.","vermelho",5000)
									return
								end

								local Allowed,Reason = vSERVER.Permission(Index,Model,Plate)
								if Allowed then
									SetEntityCoords(Vehicle, v["Coords"]["xyz"])
									SetEntityHeading(Vehicle, v["Coords"]["w"])
									Open(Vehicle, v["Logo"])
								else
									if Reason == "nopermission" then
										TriggerEvent("Notify","Negado","Você não tem permissão para usar esta oficina.","vermelho",5000)
									elseif Reason == "novehicle" then
										TriggerEvent("Notify","Negado","Não é possível modificar veículos não registrados (NPC).","vermelho",5000)
									else
										TriggerEvent("Notify","Negado","Não é possível modificar este veículo.","vermelho",5000)
									end
								end
							end
						else
...
AddEventHandler("lscustoms:Open",function()
	...
	if not Opened and IsPedInAnyVehicle(Ped) then
		...
		if GetPedInVehicleSeat(Vehicle,-1) == Ped then
			...
			local Allowed,Reason = vSERVER.Permission(Model,Plate)
			if not Allowed then
				if Reason == "novehicle" then
					TriggerEvent("Notify","Negado","Não é possível modificar veículos não registrados (NPC).","vermelho",5000)
				else
					TriggerEvent("Notify","Negado","Não é possível modificar este veículo.","vermelho",5000)
				end
