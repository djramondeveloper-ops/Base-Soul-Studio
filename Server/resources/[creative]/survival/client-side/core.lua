			...
			if GetEntityHealth(Ped) <= 100 then
				if not Death["Status"] then
					...
					if not Login then
						...
						Crawl["Timer"] = Crawl["Default"] or 1
					else
						Login = false
						Crawl["Timer"] = 1
						Death["Cooldown"] = GetGameTimer()
					...
					if AutomaticWarning then
						TriggerServerEvent("paramedic:SurvivalCall")
					end
				else
					TimeDistance = 1
					...
					if GetGameTimer() >= Death["Cooldown"] then
						...
						if Crawl["Timer"] > 0 then
							...
							if Crawl["Timer"] <= 0 then
								...
								LocalPlayer["state"]:set("Crawl",false,true)
								Death["Timer"] = Death["Default"] or 5
