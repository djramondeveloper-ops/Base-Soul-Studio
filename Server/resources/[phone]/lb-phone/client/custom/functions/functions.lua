...
function DisplayCameraButtons(buttons)
    if hudDisabled or (Config.Camera and Config.Camera.ShowTip == false) then
        return
    end
...
function GiveVehicleKey(vehicle, plate)
    -- Na integração standalone/vRP a chave real é entregue no servidor como vehiclekey-<Plate>.
