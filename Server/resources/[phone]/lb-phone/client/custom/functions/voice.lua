...
function SetCallVolume(volume)
    volume = (volume or (settings and settings.sound and settings.sound.callVolume or 0.5)) / 1
...
local voiceTargetsInterval = Interval:new(function()
    ...
    local baseVolume = (settings and settings.sound and settings.sound.volume or 0.5) / 1
    local callVolume = (settings and settings.sound and settings.sound.callVolume or 0.5) / 1
...
---@param options { sources: number | number[], audio?: boolean, phoneCall?: boolean, playFromSource?: number } | number | number[]
---@param legacyAudio? boolean
---@param legacyPhoneCall? boolean
---@param legacyPlayFromSource? number
RegisterNetEvent("phone:phone:addVoiceTarget", function(options, legacyAudio, legacyPhoneCall, legacyPlayFromSource)
    -- Seoul compatibility: older parts of this lb-phone build still emit
    -- addVoiceTarget(source, audio, phoneCall, playFromSource), while the
    -- newer voice layer expects a single options table. Support both contracts.
    if type(options) ~= "table" or options.sources == nil then
        options = {
            sources = options,
            audio = legacyAudio,
            phoneCall = legacyPhoneCall,
            playFromSource = legacyPlayFromSource
        }
    end

    if options.sources == nil then
        debugprint("Ignoring addVoiceTarget without sources")
        return
    end
    ...
    for i = 1, #sources do
        ...
        if options.phoneCall then
            if type(submixes.speaker) == "table" and options.playFromSource then
                local spatialSubmix = GetSpatialAudioSubmix(options.playFromSource)
                submix = spatialSubmix and spatialSubmix.submixId
