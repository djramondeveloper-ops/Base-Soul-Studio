...
RegisterNetEvent("QBCore:Client:OnJobUpdate", function(jobInfo)
    ...
    if oldJob.name ~= PlayerJob.name or (oldJob.grade and oldJob.grade.level) ~= (PlayerJob.grade and PlayerJob.grade.level) then
...
function GetJobGrade()
    return PlayerJob and PlayerJob.grade and PlayerJob.grade.level or 0
end
