...
local function BringCar(data, cb)
    ...
    if Config.ServerSideSpawn then
        ...
        if not vehNetId then
            debugprint("BringCar: Server did not create vehicle (no vehNetId)")
            return cb(false)
        end
    ...
    TaskVehicleDriveToCoord(ped, vehicle, plyCoords.x, plyCoords.y, plyCoords.z, 20.0, 0, GetEntityModel(vehicle), 786603, 1.0, 1)
    ...
    elseif action == "setWaypoint" then
        ...
        if coords then
            ...
            TriggerEvent("phone:sendNotification", {
                ...
                content = "O sistema de GPS do veículo está desligado, não foi possível obter sua localização.",
            })
        else
