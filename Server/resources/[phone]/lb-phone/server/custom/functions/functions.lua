...
function GiveVehicleKey(source, plate, vehicle)
    ...
    if Config.Framework == "standalone" and GrantVehicleKey then
        local success = GrantVehicleKey(source, plate)
        debugprint("GiveVehicleKey Seoul para a placa: " .. tostring(plate) .. " -> " .. tostring(success))
        return success
    end
