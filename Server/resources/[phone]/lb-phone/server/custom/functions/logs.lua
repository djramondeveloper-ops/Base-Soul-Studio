...
function Log(action, source, level, title, metadata, image)
	if not (Config.Logs and Config.Logs.Enabled) or not Config.Logs.Actions[action] then
		return
	end
...
if Config.Logs and Config.Logs.Enabled and Config.Logs.Service == "discord" then
    ---@type string?
    local defaultWebhook = LOGS and LOGS.Default
