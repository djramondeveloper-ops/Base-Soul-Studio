...
function AddMoney(source, amount)
    ...
    local result = oxCore:CallAccount(accountId, "addBalance", {
        amount = amount,
        message = "Phone"
    })
...
function RemoveMoney(source, amount)
    ...
    local result = oxCore:CallAccount(accountId, "removeBalance", {
        amount = amount,
        message = "Phone"
    })
    return result and result.success == true
...
BaseCallback("services:getManagementEmployees", function(source, phoneNumber, ...)
    ...
    local groupData = GlobalState["group." .. job]
    local grades = groupData and groupData.grades or {}
    ...
    for i = 1, #employees do
        ...
        employee.online = onlinePlayer and onlinePlayer.charId == employee.id
    end
...
BaseCallback("services:getBalance", function(source, phoneNumber)
    ...
    local accountId = groupAccount and groupAccount.accountId
...
BaseCallback("services:hireEmployee", function(source, phoneNumber, targetSource)
    ...
    local groupData = GlobalState["group." .. job]
    local amountGrades = #(groupData and groupData.grades or {})
