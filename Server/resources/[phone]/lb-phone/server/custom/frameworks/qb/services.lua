...
function GetEmployees(job)
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData and v.PlayerData.job and v.PlayerData.job.name == job and v.PlayerData.job.onduty then
...
function RefreshCompanies()
    if Config.QBOldJobMethod then
        ...
        for _, v in pairs(players) do
            if not (v and v.PlayerData and v.PlayerData.job and v.PlayerData.job.onduty) then
                goto continue
            end
