...
if not (Config.DatabaseChecker and Config.DatabaseChecker.Enabled) and not shouldGenerateTables then
...
local function ValidateNotificationsId()
    local idColumn = tables.phone_notifications and tables.phone_notifications.id

    if idColumn and idColumn.type == "VARCHAR" then
        MySQL.rawExecute.await([[
