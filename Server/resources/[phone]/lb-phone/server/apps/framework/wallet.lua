...
BaseCallback("wallet:sendPayment", function(source, phoneNumber, data)
    ...
    elseif Config.TransferLimits and Config.TransferLimits.Daily then
    ...
    elseif Config.TransferLimits and Config.TransferLimits.Weekly then
    ...
    if (sendToSource and sendToSource == source) or (sendToIdentifier and sendToIdentifier == identifier) then
        debugprint("Can't send money to yourself.")
        return { success = false, reason = "FAILED_ADD" }
    end

    -- REMOVER PRIMEIRO (Evitar duplicação em caso de erro)
    ...
    elseif Config.TransferOffline and sendToIdentifier then
        added = AddMoneyOffline and AddMoneyOffline(sendToIdentifier, amount)
    end

    -- SE FALHAR, DEVOLVER AO PAGADOR
    if not added then
        AddMoney(source, amount) -- Refund
        return { success = false, reason = "FAILED_ADD" }
    end
